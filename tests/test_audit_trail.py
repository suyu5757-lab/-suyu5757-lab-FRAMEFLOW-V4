...
class AuditTrailCoverageTests(unittest.TestCase):
    ...
    def test_prompt_mutation_has_reason_and_redacts_secret_like_fields(self) -> None:
        ...
        self.assertTrue(event["after"]["prompt"].startswith("A quiet room"))
        self.assertIn("横向空环境参考板", event["after"]["prompt"])
        self.assertNotIn("Prompt Contract", event["after"]["prompt"])
        self.assertEqual(event["result"], "success")
