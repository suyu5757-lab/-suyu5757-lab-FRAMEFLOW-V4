...
def visual_prompt(asset_class: str, subject: str, visual: str, *, action: str = "") -> dict:
...
def fusion_project(project_id: str = "PRJ_FUSION") -> dict:
    ...
    return {
        ...
        "assets": [
            {"id": "C001", "name": "前景角色", "assetClass": "character", "grade": "B", **visual_prompt("character", "前景角色的稳定身份参考", "写实角色设计参考，清楚表现脸部、发型、体型和服装材质。"), **ready_fields},
            {"id": "C002", "name": "中景角色", "assetClass": "character", "grade": "B", **visual_prompt("character", "中景角色的稳定身份参考", "写实角色设计参考，清楚表现脸部、发型、体型和服装材质。"), **ready_fields},
            {"id": "S001", "name": "山路环境", "assetClass": "scene", "grade": "B", **visual_prompt("scene", "山路环境的稳定空间参考", "写实场景设计参考，湿石路面、山体植被与自然光线。"), **ready_fields},
...
def prompt_card(asset_id: str, asset_class: str) -> dict:
    compiled = visual_prompt(
        asset_class,
        f"{asset_id} 的正式视觉资产",
        "写实制作参考，清楚呈现主体结构、材质和光线。",
        action="保持主体动作、摄影机关系和连续性。" if asset_class in {"fusion", "shot"} else "",
    )
    return {
        "id": asset_id,
...
class FusionPromptFlowTests(unittest.TestCase):
    ...
    def test_fusion_gate_reports_prompt_qa_blockers_and_opens_after_all_sources_are_ready(self) -> None:
        ...
        sources = [
            {"id": "ENV01", "name": "ENV01", "assetClass": "scene", **visual_prompt("scene", "ENV01 环境", "写实场景设计参考。"), "promptQaDecision": "Pending", **ready_fields},
            {"id": "P01", "name": "P01", "assetClass": "character", **visual_prompt("character", "P01 角色", "写实角色设计参考。"), "promptQaDecision": "Pending", **ready_fields},
            {"id": "P02", "name": "P02", "assetClass": "character", **visual_prompt("character", "P02 角色", "写实角色设计参考。"), "promptQaDecision": "Pending", **ready_fields},
        ]
    ...
    def test_prerequisite_gate_derives_prompt_pack_dependencies_and_blocks_intake(self) -> None:
        ...
        project["assets"] = [
            {"id": "P01", "name": "P01", "assetClass": "character", **visual_prompt("character", "P01 稳定角色身份", "写实角色设计参考，清楚呈现身份、服装和材质。"), **{
                "artifactId": "ART_P01", "qaDecision": "Approved", "regulatorRegistered": True,
                "status": "ready", "promptQaDecision": "Approved",
            }},
            {"id": "ENV01", "name": "ENV01", "assetClass": "scene", **visual_prompt("scene", "ENV01 稳定环境空间", "写实场景设计参考，清楚呈现空间结构和材质。"), **{
                "artifactId": "ART_ENV01", "qaDecision": "Approved", "regulatorRegistered": True,
                "status": "ready", "promptQaDecision": "Approved",
            }},
            {"id": "P12", "name": "P12", "assetClass": "prop", **visual_prompt("prop", "P12 独立道具", "清晰的独立物件设计参考，明确结构和材质。")},
            {
                "id": "P08", "name": "P08", "assetClass": "prop", **visual_prompt("prop", "P08 独立物件", "清晰的独立物件设计参考，明确结构和材质。"),
                "promptQaDecision": "Approved",
            },
        ]
        project["assetRegulator"] = {"dependencyTable": [
            {"from": "P01", "to": ["P08"], "reason": "沿用 P01 的身份与接口连续性"},
            {"from": "ENV01", "to": ["P08"], "reason": "沿用环境空间连续性"},
            {"from": "P12", "to": ["P08"], "reason": "等待实体命中承载物"},
        ]}
    ...
    def test_existing_fusion_slot_auto_links_and_generates_without_manual_edges(self) -> None:
        ...
        with mock.patch.object(server, "_run_fusion_prompt_agent", new=mock.AsyncMock(return_value={
            ...
            "promptPack": visual_prompt("fusion", "两名角色与山路环境的统一空间画面", "统一写实视觉语言、材质响应、环境光和投射阴影。", action="三人沿山路押解，前景角色回头观察；摄影机保持山路轴线和前中后景连续。共用同一透视、遮挡和地面接触关系。")["promptPack"],
    ...
    def test_targeted_generation_records_lineage_and_stale_state(self) -> None:
        ...
        with mock.patch.object(server, "_run_fusion_prompt_agent", new=mock.AsyncMock(return_value={
            ...
            "promptPack": visual_prompt("fusion", "三人沿山路押解的统一空间画面", "统一写实视觉语言、材质响应、环境光和投射阴影。", action="三人沿山路押解，保持前中后景空间关系、山路轴线和主体连续性。")["promptPack"],
    ...
    def test_fusion_prompt_stays_usable_when_output_image_is_replaced(self) -> None:
        ...
        with mock.patch.object(server, "_run_fusion_prompt_agent", new=mock.AsyncMock(return_value={
            ...
            "promptPack": visual_prompt("fusion", "三人沿山路押解的统一空间画面", "统一写实视觉语言、材质响应、环境光和投射阴影。", action="三人沿山路押解，保持空间连续、相对尺度、遮挡关系和山路轴线。")["promptPack"],
            "mustPreserve": ["角色身份", "山路轴线"],
            "mustAvoid": ["重复人物"],
            "warnings": [],
        })):
