...
class FrameflowV3SettingsTests(unittest.TestCase):
    ...
    def test_credential_write_import_clear_and_probe_never_echo_secret(self) -> None:
        ...
        with mock.patch.object(server, "set_secret") as write, mock.patch.object(server, "get_secret", return_value=secret), mock.patch.dict(server.os.environ, {"OPENCODE_SERVER_PASSWORD": secret}, clear=False):
            response = self.client.post("/api/v2/settings/providers/opencode-default/credential", json={"api_key": secret})
            self.assertEqual(response.status_code, 200, response.text)
            write.assert_called_once()
            self.assertNotIn(secret, response.text)
            imported = self.client.post("/api/v2/settings/providers/opencode-default/credential/import", json={"environment_variable": "OPENCODE_SERVER_PASSWORD"})
            self.assertEqual(imported.status_code, 200, imported.text)
            self.assertNotIn(secret, imported.text)
            with mock.patch.object(server, "delete_secret"):
                cleared = self.client.delete("/api/v2/settings/providers/opencode-default/credential")
        ...
        with mock.patch.object(server, "probe_profile", new=mock.AsyncMock(return_value=probe_result)), mock.patch.object(server, "get_profile_secret", return_value="probe-secret"):
            probed = self.client.post("/api/v2/settings/providers/opencode-default/probe")
        ...
        models = self.client.get("/api/v2/settings/providers/opencode-default/models")
    ...
    def test_failed_probe_is_persisted_as_the_latest_health_result(self) -> None:
        ...
        settings = self.client.get("/api/v2/settings").json()
        provider = next(item for item in settings["providers"] if item["id"] == "opencode-default")
    ...
    def test_capability_binding_rejects_disabled_provider_and_accepts_v3_binding(self) -> None:
        ...
        created = self.client.post("/api/v2/settings/providers", json={
            "id": "disabled-settings-provider", "provider_type": "comfyui", "display_name": "停用接口", "base_url": "http://127.0.0.1:8188", "capabilities": ["orchestrator"], "enabled": False,
        })
        self.assertEqual(created.status_code, 200, created.text)
    ...
    def test_auto_matching_repairs_disabled_route_and_accepts_official_opencode_go_model(self) -> None:
        disabled = self.client.patch("/api/v2/settings/providers/minimax-default", json={"enabled": False})
        self.assertEqual(disabled.status_code, 200, disabled.text)
        updated = self.client.patch("/api/v2/settings/providers/opencode-default", json={
            "model_config": {"orchestrator_model": "opencode-go/gpt-5.6-luna", "thinking_strength": "max", "agent": "build"},
        })
        self.assertEqual(updated.status_code, 200, updated.text)
        ...
        self.assertEqual((binding["provider_profile_id"], binding["model"]), ("opencode-default", "opencode-go/gpt-5.6-luna"))
    ...
    def test_preset_and_validation_boundaries(self) -> None:
        ...
        invalid = self.client.post("/api/v2/settings/providers", json={
            "provider_type": "opencode", "display_name": "错误地址", "base_url": "http://remote.example/v1", "capabilities": [],
        })
