...
import server
from frameflow.prompt_design import canonicalize_prompt_output, select_style_template
...
def style_pack(asset_class: str, subject: str, visual: str = "清晰统一的视觉语言。", *, action: str = "") -> dict:
    raw = {"subjectTask": subject, "visualStyleMaterials": visual}
    if action:
        raw["actionContinuity"] = action
...
class AssetBoardV4Tests(unittest.TestCase):
    ...
    def test_confirmed_asset_intents_are_passed_to_the_existing_prompt_pipeline(self) -> None:
        ...
        def pack(asset_class: str) -> dict:
            return style_pack(asset_class, f"{asset_class} 的生产设定", "写实制作参考，克制光线，清楚呈现主体结构与材质。")
        ...
        prompt_output = {
            "assets": [
                {"id": "CHAR_01", "name": "陈继业", "assetClass": "character", "priority": "B", "required": True, "targetSkill": "video-character-design-director", "relevantShots": ["S001"], "prompt": "角色 Prompt", "promptPack": pack("character"), "mustPreserve": [], "mustAvoid": []},
                {"id": "SCENE_01", "name": "祠堂", "assetClass": "scene", "priority": "B", "required": True, "targetSkill": "video-scene-design-director", "relevantShots": ["S001"], "prompt": "环境 Prompt", "promptPack": pack("scene"), "mustPreserve": [], "mustAvoid": []},
                {"id": "PROP_01", "name": "百物录", "assetClass": "prop", "priority": "B", "required": True, "targetSkill": "video-prop-design-director", "relevantShots": ["S002"], "prompt": "道具 Prompt", "promptPack": pack("prop"), "mustPreserve": [], "mustAvoid": []},
            ],
        ...
        self.assertEqual(library["CHAR_01"].get("promptCompositionMode"), "style_library")
        self.assertEqual(library["SCENE_01"].get("promptCompositionMode"), "style_library")
        self.assertEqual(library["PROP_01"].get("promptCompositionMode"), "style_library")
    ...
    def test_targeted_prompt_generation_returns_editor_draft_without_saving_prompt(self) -> None:
        ...
        prompt_output = {
            "assets": [{"id": "SCENE_01", "name": "祠堂", "assetClass": "scene", "prompt": "雨夜祠堂的完整场景 Prompt", "promptPack": style_pack(
                "scene",
                "建立可跨镜头复用的雨夜祠堂场景，雨水在湿石阶上形成连续反射",
                "写实场景设计参考；吸水石材表面有细密雨水膜，冷蓝雨夜环境光与檐下暖光形成可见反射。",
            ), "relevantShots": ["S001"]}],
            "fusionPlans": [],
        }
        ...
        self.assertEqual(payload["run"]["promptCards"][0]["promptContractVersion"], "3.0-style-library")
        self.assertEqual(payload["run"]["promptCards"][0]["promptWorkflow"], "style-library-visual-v1")
        ...
        self.assertEqual(character.get("prompt"), "")
        self.assertTrue(character.get("needsRegeneration"))
    ...
    def test_targeted_prompt_generation_passes_operator_idea_to_both_ai_stages(self) -> None:
        ...
        prompt_output = {
            "assets": [{"id": "SCENE_01", "name": "祠堂", "assetClass": "scene", "prompt": "压低雨水反射后的祠堂场景 Prompt", "promptPack": style_pack(
                "scene",
                "建立一座可跨镜头复用的雨夜祠堂，雨水在湿石阶上形成连续反射",
                "湘西乡村祠堂；吸水石材和旧木表面有细密雨水膜，冷蓝雨夜环境光与檐下暖光形成可见反射。",
            ), "relevantShots": ["S001"]}],
            "fusionPlans": [],
        }
        captured: dict[str, dict] = {}
    ...
    def test_clean_prompt_save_replaces_old_pack_without_appending_it(self) -> None:
        def scene_pack(intent: str, surface: str) -> dict:
            return style_pack(
                "scene",
                intent,
                f"写实场景设计参考；{surface}；冷蓝雨夜环境光与檐下暖光形成可见反射。",
            )
    ...
    def test_c001_character_rewrite_is_one_clean_sheet_and_moves_shot_actions_out(self) -> None:
        ...
        prompt_pack = style_pack(
            "character",
            "建立 C001 可跨镜头复用的单张角色结构参考板；静态展示同一角色的面部与上半身特写、正面全身、侧面全身和背面全身视图",
            "真人可信的角色设计参考板；柔和鹅蛋脸、黑色偏冷棕中短发、珍珠白或冷白高性能驾驶服、石墨黑辅助结构、白色驾驶手套；中性均匀棚拍光线清楚表现皮肤、发丝、服装接缝、薄型装甲和手套材质差异。",
        )
        prompt_output = {
            "assets": [{
                "id": "CHAR_01",
                ...
                "relevantShots": ["S001"],
                "mustPreserve": ["白色驾驶服", "白色驾驶手套", "SH001 与 SH002 连续性"],
        ...
        self.assertEqual(next(asset for asset in payload["library"]["assets"] if asset["id"] == "CHAR_01").get("prompt"), "")
        self.assertTrue(next(asset for asset in payload["library"]["assets"] if asset["id"] == "CHAR_01").get("needsRegeneration"))
    ...
    def test_provider_component_suffixes_fold_back_into_stable_asset_id(self) -> None:
        repaired = server._repair_prompt_card_ids(
            {
                "assets": [
                    {"id": "P08A", "assetClass": "prop", "name": "P08A 实体手里剑", "prompt": "三枚实体手里剑", "promptPack": style_pack("prop", "三枚实体手里剑", "磨砂金属材质，蓝紫能量边缘。"), "relevantShots": ["S04"], "mustPreserve": ["三枚实体"], "mustAvoid": []},
                    {"id": "P08B", "assetClass": "vfx", "name": "P08B 复制光轮", "prompt": "蓝紫复制光轮", "promptPack": style_pack("prop", "蓝紫复制光轮", "蓝紫能量边缘，独立物件结构清楚。"), "relevantShots": ["S05"], "mustPreserve": ["复制光轮"], "mustAvoid": ["可数实体堆叠"]},
                ],
                "warnings": [],
            },
            {"P08"},
        ...
        self.assertEqual([item["sourceId"] for item in card["componentDetails"]], ["P08A", "P08B"])
    ...
    def test_prompt_contract_pack_is_saved_and_projected_with_quality_coverage(self) -> None:
        prompt_pack = style_pack(
            "scene",
            "雨夜祠堂的连续场景，保留湿石阶、院落动作区和檐下暖灯",
            "写实场景设计参考；吸水石材、旧木与细雨水膜，雨夜冷蓝环境光与檐下暖灯形成可见反射。",
        )
        ...
        self.assertEqual(scene["promptPack"]["templateSelection"]["templateId"], "architecture-space")
        self.assertEqual(set(scene["promptPack"]), {
            "templateSelection", "subjectTask", "compositionLayout", "visualStyleMaterials",
            "textLabels", "aspectRatioOutput", "constraintsNegative", "referenceRoles",
        })
        self.assertEqual(scene["promptContractVersion"], "3.0-style-library")
