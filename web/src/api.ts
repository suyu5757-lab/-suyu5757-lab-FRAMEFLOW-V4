import type { AgentPlan, AudioStudioEnvelope, AudioStudioDocument, AssetAuditEnvelope, AssetBoard, AssetBoardEnvelope, AssetIntentEnvelope, AssetIntentPrepareEnvelope, AssetLibraryEnvelope, AssetPromptRunEnvelope, DashboardEnvelope, FusionPromptRunEnvelope, GraphEnvelope, MiniMaxRegion, MiniMaxVoiceCatalog, ProjectCreateInput, ProjectRecord, RenderEstimate, RenderJob, RunEstimate, SettingsEnvelope, SettingsProvider, SpeechGenerateInput, StoryDiff, StoryDocument, StoryEnvelope, StoryRun, TimelineDocument, TimelineEnvelope, TimelinePreflight, VoiceDesignGenerateInput, WorkflowGraph, WorkflowManifest, WorkflowRun, WorkflowRunDetail } from './types';
...
export const studioApi = {
  ...
  generateAssetPrompts: (projectId: string, body: { expected_revision?: number; target_asset_id?: string; review_feedback?: string; source_qa_run_id?: string; operator_idea?: string; asset_intent_version?: number; template_override?: string }) => request<AssetPromptRunEnvelope>(`/api/v2/projects/${encodeURIComponent(projectId)}/asset-prompt-runs`, json('POST', body)),
