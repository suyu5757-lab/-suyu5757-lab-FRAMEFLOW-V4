...
const defaultSeedancePlan = (duration: number, generator: string, aspectRatio = '16:9') => ({
...
const SCENE_LEDGER_FIELDS = ['id', 'name', 'description', 'interiorExterior', 'timeOfDay', 'location', 'characterIds', 'propIds', 'narrativeFunction', 'emotion', 'visualAnchors', 'spatialGeography', 'materialEvidence', 'lightingCausality', 'soundscape', 'productionDifficulty', 'relevantShots', 'sceneGoal', 'characterDesire', 'primaryObstacle', 'observableAction', 'stateChange', 'turn', 'entryState', 'exitState'];
...
export function StoryWorkbench({ projectId, story, storyRun, dirty, busy, notice, assetPromptRun, onChange, onSave, onGenerateOptimized, onGenerateStoryboard, onGenerateStoryboardFromScript, onRegenerate, onAccept, onRollback, onOpenAssetBoard, onPrepareAssetIntent, onGenerateAssetPrompts }: StoryWorkbenchProps) {
  ...
  const candidateAutoReview = (output.autoReview && typeof output.autoReview === 'object' ? output.autoReview : {}) as Record<string, unknown>;
  const candidateAutoReviewStatus = String(candidateAutoReview.status || '');
  const candidateAutoReviewGates = Array.isArray(candidateAutoReview.gates) ? candidateAutoReview.gates.filter((item): item is Record<string, unknown> => Boolean(item && typeof item === 'object')) : [];
  const candidateAutoReviewIssues = Array.isArray(candidateAutoReview.issues) ? candidateAutoReview.issues.filter((item): item is Record<string, unknown> => Boolean(item && typeof item === 'object')) : [];
  const candidateAutoReviewBlocking = candidateAutoReviewIssues.filter((item) => item.severity === 'blocking');
  const candidateAutoReviewWarnings = candidateAutoReviewIssues.filter((item) => item.severity === 'warning');
  const candidateAutoReviewNeedsReview = candidateAutoReviewStatus === 'needs_user_review';
  const candidateIncomplete = output.acceptanceAllowed === false || output.handoffStatus === 'handoff_not_ready' || candidateAutoReviewStatus === 'blocked' || candidateAutoReviewNeedsReview || candidateCoverage.status === 'incomplete' || candidateDroppedItems.length > 0 || candidateLegacyUnverified;
  ...
  const candidateIssueSummary = candidateIncomplete ? [
    ...
    candidateAutoReviewBlocking.length ? `导演审查阻断 ${candidateAutoReviewBlocking.length} 项` : '',
    candidateAutoReviewNeedsReview ? '存在需要用户确认的新增内容' : '',
  ...
  const addShot = () => updateDocument((current) => {
    ...
    const newShot: StoryShot = { id, scene, duration: 8, purpose: '', size: '中景', camera: '固定或轻微推进', action: '', visibleEvent: '', eventConsequence: '', subjectFocus: '', performance: '', blocking: '', coverageRole: '', environmentPressure: '', microAction: '', visualMotif: '', dialogue: '', narration: '', sound: '', environment: '', spatialGeography: '', materialEvidence: '', lightingCausality: '', cameraExecution: '', atmosphereBehavior: '', generationMethod: 'reference_to_video', difficulty: 'medium', risks: [], referenceRoles: [], assetRequirements: [], seedancePlan: defaultSeedancePlan(8, spec.generator_profile || '', spec.ratio || '16:9'), continuity: defaultContinuity() };
  ...
  return <section className="story-workbench-v2" aria-busy={busy}>
    ...
    <section className="story-v2-stage story-v2-board-stage">
      ...
      <div className="story-v2-scene-stack">{shotGroups.map(({ scene, shots: sceneShots }) => { const sceneId = String(scene.id || 'UNASSIGNED'); return <article className="story-v2-scene-card" data-story-scene-id={sceneId} key={sceneId}>
        ...
        {sceneId !== 'UNASSIGNED' && <details className="story-v2-scene-details"><summary><span>场景账本详情</span><small>空间、戏剧目标、资产、声音与连续性字段</small><b>查看详情</b></summary><div className="story-v2-scene-fields"><div className="story-v2-field-grid">{sceneField(scene, 'name', '场景名称')}{sceneField(scene, 'description', '场景摘要', true)}{sceneField(scene, 'interiorExterior', '内外景')}{sceneField(scene, 'timeOfDay', '时间')}{sceneField(scene, 'location', '地点')}{sceneField(scene, 'emotion', '情绪')}{sceneField(scene, 'narrativeFunction', '叙事功能')}{sceneField(scene, 'productionDifficulty', '生产难度')}</div><div className="story-v2-field-grid wide">{sceneField(scene, 'sceneGoal', '场景目标', true)}{sceneField(scene, 'characterDesire', '人物当前欲望', true)}{sceneField(scene, 'primaryObstacle', '主要阻碍', true)}{sceneField(scene, 'observableAction', '可观察行动', true)}{sceneField(scene, 'stateChange', '状态变化', true)}{sceneField(scene, 'turn', '场景转折', true)}{sceneField(scene, 'entryState', '进入状态', true)}{sceneField(scene, 'exitState', '退出状态', true)}{sceneField(scene, 'characterIds', '人物 ID（每行一个）', true)}{sceneField(scene, 'propIds', '道具 ID（每行一个）', true)}{sceneField(scene, 'relevantShots', '关联镜头（每行一个）', true)}{sceneField(scene, 'spatialGeography', '空间地理', true)}{sceneField(scene, 'visualAnchors', '视觉锚点（每行一个）', true)}{sceneField(scene, 'materialEvidence', '材质证据', true)}{sceneField(scene, 'lightingCausality', '光线因果', true)}{sceneField(scene, 'soundscape', '环境声音', true)}</div></div></details>}
    ...
    {storyboardReviewReady && <section className="story-v2-review">
      ...
      {candidateAutoReviewStatus && candidateAutoReviewStatus !== 'not_applicable' && <section className={`story-v2-auto-review ${candidateAutoReviewStatus === 'blocked' ? 'blocked' : candidateAutoReviewStatus === 'passed_with_warnings' || candidateAutoReviewNeedsReview ? 'warning' : 'passed'}`} role={candidateAutoReviewStatus === 'blocked' || candidateAutoReviewNeedsReview ? 'alert' : 'status'}><div><strong>自动导演审查</strong><span>{candidateAutoReviewStatus === 'passed' ? '已通过' : candidateAutoReviewStatus === 'passed_with_warnings' ? `通过并有 ${candidateAutoReviewWarnings.length} 项提醒` : candidateAutoReviewNeedsReview ? '需要用户确认新增内容' : candidateAutoReviewStatus === 'blocked' ? `已阻断 · ${candidateAutoReviewBlocking.length} 项待修复` : candidateAutoReviewStatus}</span><small>{String(candidateAutoReview.ruleProfile || output.ruleProfile || 'legacy')} · 修复尝试 {Number(candidateAutoReview.repairAttempts || 0)}/1</small></div><div className="story-v2-auto-review-gates">{candidateAutoReviewGates.map((gate, index) => <span className={String(gate.status || '')} key={String(gate.id || gate.label || index)}>{String(gate.label || gate.id || '审查门')} · {String(gate.status || 'pending')}</span>)}</div></section>}
      ...
      {candidateIncomplete ? <div className="story-v2-review-blocker" role="alert"><strong>⚠ 分镜候选不完整，暂不能接受</strong><span>{candidateIssueSummary}</span><details><summary>查看缺失内容与结构诊断</summary>{candidateLegacyUnverified && <div className="story-v2-review-diagnostic"><b>旧候选</b><span>该候选由旧规则生成，尚未完成原文节拍覆盖检查；请重新生成分镜后再接受。</span></div>}{candidateMissingBeats.length > 0 && <div className="story-v2-review-diagnostic"><b>缺失或部分覆盖的剧本节拍</b><ul>{candidateMissingBeats.map((item, index) => <li key={`${String(item.beatId || 'beat')}-${index}`}><code>{String(item.beatId || '未知')}</code><span>{String(item.summary || item.reason || '未说明的剧本内容')}</span></li>)}</ul></div>}{candidateDroppedItems.length > 0 && <div className="story-v2-review-diagnostic"><b>无法解析的镜头条目</b><ul>{candidateDroppedItems.map((item, index) => <li key={`${String(item.path || 'item')}-${index}`}><code>{String(item.path || 'provider output')}</code><span>{String(item.reason || '条目格式不合法')}</span></li>)}</ul></div>}{candidateAutoReviewIssues.length > 0 && <div className="story-v2-review-diagnostic"><b>自动导演审查</b><ul>{candidateAutoReviewIssues.map((item, index) => <li key={`${String(item.code || 'director')}-${index}`}><code>{String(item.code || 'director_review')}</code><span>{String(item.message || '')}</span></li>)}</ul></div>}{candidateBlockingIssues.length > 0 && <div className="story-v2-review-diagnostic"><b>系统校验结果</b><ul>{candidateBlockingIssues.map((item, index) => <li key={`${String(item.code || 'issue')}-${index}`}><code>{String(item.code || 'issue')}</code><span>{String(item.message || '')}</span></li>)}</ul></div>}</details></div> : <div className="story-v2-review-ready">✓ 分镜已通过剧本覆盖检查，可继续审核。</div>}
