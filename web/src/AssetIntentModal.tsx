  ...
  return <div className="asset-intent-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !savingDraft && !generating) onClose(); }}>
    <section className="asset-intent-dialog" role="dialog" aria-modal="true" aria-labelledby="asset-intent-title" onMouseDown={(event) => event.stopPropagation()}>
      ...
      {!loading && <>
        ...
        {envelope?.currentProvenance?.sourceStoryboardVersionId && <div className="asset-intent-provenance" role="status"><span>来源分镜 {envelope.currentProvenance.sourceStoryboardVersionId}</span><span>关联镜头 {envelope.currentProvenance.sourceShotIds?.length ? envelope.currentProvenance.sourceShotIds.join('、') : '待确认'}</span></div>}
