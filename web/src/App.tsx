...
import { buildMiniMaxWebPromptPackage, formatMiniMaxWebPromptPackage, normalizePromptPack } from './prompt-design';
...
const STYLE_LIBRARY_TEMPLATE_OPTIONS = [
...
function AssetBoardToolbar({
 ...
 export function assetBoardToFlowNodes(board: AssetBoard, assets: LibraryAsset[], filter: string, showShots: boolean, storyShots: StoryShot[] = [], options: { forceGrid?: boolean; preset?: AssetGridPreset; columnWidth?: number; columnWidths?: AssetBoardColumnWidths; gap?: number; layoutMode?: AssetBoardLayoutMode; collapsedScopes?: Record<string, string | true>; onlyBlocked?: boolean; showCandidates?: boolean; shotId?: string; selectedSelectionKey?: AssetBoardSelectionKey | null; onToggleScope?: (target: AssetBoardCollapseTarget) => void; onContextMenu?: (target: AssetBoardContextTarget) => void; onApprovePrompt?: (assetId: string) => void; onGeneratePrompt?: (assetId: string, operatorIdea?: string) => void; onGenerateFusionPrompt?: (assetId: string, sourceAssetIds: string[], shotId: string) => void; onCopyPrompt?: (assetId: string) => void; onUploadAsset?: (assetId: string, file: File) => void; onRemoveArtifact?: (assetId: string, artifactId: string) => void; onApproveAsset?: (assetId: string, artifactId: string) => void; onRejectAsset?: (assetId: string, artifactId: string) => void; onRegisterAsset?: (assetId: string, artifactId: string) => void; onColumnResize?: (key: keyof AssetBoardColumnWidths, delta: number) => void; onOpenAssetProduction?: (assetId: string, target: AssetProductionTarget, nodeId?: string) => void } = {}): AssetFlowNode[] {
  ...
  for (const node of board.nodes) {
    ...
    // Legacy boards may still contain the old generic bridge. It is
    // intentionally hidden here; the Prompt card is the single handoff
    // surface for copying a prompt and uploading an external candidate.
...
function assetBoardFromFlow(board: AssetBoard, nodes: AssetFlowNode[], edges: Edge[]): AssetBoard {
  ...
  for (const node of nodes.filter((candidate) => !candidate.data.presentationOnly && candidate.data.node_type !== 'row' && candidate.data.node_type !== 'table')) {
    const { onToggleScope, onContextMenu, onApprovePrompt, onGeneratePrompt, onGenerateFusionPrompt, onCopyPrompt, onColumnResize, onOpenAssetProduction, collapsed, presentationOnly, sourceNodeId, ...persistedData } = node.data;
    void onToggleScope; void onContextMenu; void onApprovePrompt; void onGeneratePrompt; void onGenerateFusionPrompt; void onCopyPrompt; void onColumnResize; void onOpenAssetProduction; void collapsed; void presentationOnly; void sourceNodeId;
...
function composeAudioPrompt(asset: LibraryAsset, story: StoryEnvelope | null, prompt: string): string {
  ...
  if (asset.assetClass === 'audio') {
    const webPackage = buildMiniMaxWebPromptPackage(asset.promptPack || metadata.prompt_pack || {}, prompt, { shots, references: asset.references || [] });
    return formatMiniMaxWebPromptPackage(webPackage, asset.name || '', asset.id);
  }
  // The browser only packages the independent MiniMax audio workflow. Visual
  // copy text is always returned by the backend as copyablePrompt.
  void metadata;
  void shots;
  return '';
}
...
function PromptDetailsPanel({ asset }: { asset: LibraryAsset; promptPack?: Record<string, unknown>; promptQuality?: LibraryAsset['promptQuality'] }) {
  // Visual Prompt Pack metadata is an internal compiler/QA record. The
  // production panel keeps it out of the user-facing surface; users only
  // need the copyable Prompt, fixed specs, upload and QA actions.
  void asset;
  return null;
}
...
function AssetProductionPanel({ asset, story, fusionSources, busy, projectRevision, assetBoardDirty, promptDraft, promptPackDraft, promptQualityDraft, selectedCardType, onSave, onCopyPrompt, onImport, onStartQa, onApprove, onRegister, onApprovePromptCard, onGeneratePrompt, onGenerateFusionPrompt, onManualProductionApproval, onDraftChange }: { asset?: LibraryAsset; story: StoryEnvelope | null; fusionSources: LibraryAsset[]; busy: boolean; projectRevision?: number; assetBoardDirty: boolean; promptDraft?: string; promptPackDraft?: Record<string, unknown>; promptQualityDraft?: LibraryAsset['promptQuality']; selectedCardType?: 'asset' | 'handoff' | 'artifact'; onSave: (assetId: string, body: Record<string, unknown>) => void; onCopyPrompt: (asset: LibraryAsset, prompt: string) => void; onImport: (asset: LibraryAsset, file: File) => void; onStartQa: (artifactId: string, qaType?: AssetQaType) => void; onApprove: (artifactId: string) => void; onRegister: (artifactId: string) => void; onApprovePromptCard: (assetId: string) => void; onGeneratePrompt?: (assetId: string, operatorIdea?: string, templateOverride?: string) => void; onGenerateFusionPrompt: (assetId: string, sourceAssetIds: string[], shotId: string) => void; onManualProductionApproval?: (assetId: string, approved: boolean, reason: string, artifactId: string) => void; onDraftChange?: (assetId: string, draft: AssetEditorDraft) => void }) {
  ...
  useEffect(() => {
    ...
    const nextDraft: AssetEditorDraft = {
      prompt: promptDraft !== undefined ? promptDraft : typeof stored.prompt === 'string' ? stored.prompt : String(asset.copyablePrompt || asset.prompt || ''),
  ...
  // A targeted generation response is a backend-authored, unsaved draft. It
  // is safe to copy that exact response; a locally edited textarea value is
  // not treated as an authoritative visual Prompt until it has been saved
  // and reloaded from the backend.
  const backendCopyablePrompt = promptDraft !== undefined ? promptDraft : String(asset.copyablePrompt || '');
...
function AssetPromptDraftNarrowModal({ asset, draft, busy, onClose, onSave }: { asset: LibraryAsset; draft: { assetId: string; prompt: string }; busy: boolean; onClose: () => void; onSave: (prompt: string) => void }) {
  ...
  return <div className="modal-backdrop asset-prompt-draft-narrow-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section className="confirm-dialog asset-prompt-draft-narrow-modal" role="dialog" aria-modal="true" aria-labelledby="asset-prompt-draft-title">
      ...
      <p>当前内容是未保存的 Prompt 草稿。确认后保存才会创建新的 Prompt 版本；Prompt QA、外部候选上传和媒体 QA 仍按原流程执行。</p>
...
function Studio() {
  ...
  const copyAssetPromptCard = async (assetId: string) => {
    ...
    if (!asset) { setNotice('当前资产不存在。'); return; }
    if (asset.assetClass !== 'audio' && !String(asset.copyablePrompt || '').trim()) { setNotice('当前资产没有后端编译完成的可复制 Prompt。'); return; }
    if (asset.assetClass === 'audio') {
      const metadata = asset.assetMetadata || {};
    ...
    // The backend is the single source of truth for visual copy text. Keep
    // the local compiler out of the copy action so UI code cannot resurrect
    // retired Prompt concatenation rules.
    const fullPrompt = asset.assetClass === 'audio'
      ? composeAudioPrompt(asset, story, asset.prompt)
      : String(asset.copyablePrompt || '').trim();
    try {
  ...
  const approveAssetPromptCard = async (assetId: string) => {
    ...
    if (!(await requestConfirmation('确认 Prompt QA', `确认通过「${asset?.name || assetId}」的 Prompt QA？通过后即可复制 Prompt，并在外部图像工具生成候选图。`, '通过 QA'))) return;
    setBusy(true);
    try {
      ...
      setAssetLibrary(library); setProjects(projectEnvelope.projects); await refreshPromptBoard(library); setNotice(`Prompt QA 已通过 · ${asset?.name || assetId} · 可复制 Prompt 并上传外部候选图`);
  ...
  useEffect(() => {
    ...
    studioApi.loadProjectSnapshot(projectId, controller.signal)
      ...
      const boardNodes = assetBoardToFlowNodes(assetBoard.board, library.assets, 'all', true, storyEnvelope.story.shots, { preset: loadedPreset, columnWidth: loadedColumnWidth, gap: loadedGap, layoutMode: loadedLayoutMode, collapsedScopes: {}, onToggleScope: toggleAssetBoardScope, onContextMenu: openAssetContextMenu, onApprovePrompt: approveAssetPromptCard, onCopyPrompt: copyAssetPromptCard, onUploadAsset: uploadAssetFromBoard, onApproveAsset: approveAssetFromBoard, onRejectAsset: rejectAssetFromBoard, onRegisterAsset: registerAssetFromBoard, onRemoveArtifact: removeUploadedAssetFromBoard, onOpenAssetProduction: openAssetProductionShortcut });
  ...
  const refreshAssetBoard = async (preserveLayout = true, libraryOverride?: AssetLibraryEnvelope, selectedAssetId?: string | null) => {
    ...
    const boardNodes = assetBoardToFlowNodes(refreshed.board, assets, assetBoardFilter, assetBoardShowShots, story?.story.shots || [], { preset: assetBoardLayoutPreset, columnWidth: assetBoardColumnWidth, gap: assetBoardGap, layoutMode: assetBoardLayoutMode, collapsedScopes: assetBoardCollapsedScopes, onlyBlocked: assetBoardOnlyBlocked, showCandidates: assetBoardShowCandidates, shotId: assetBoardShotId, onToggleScope: toggleAssetBoardScope, onContextMenu: openAssetContextMenu, onApprovePrompt: approveAssetPromptCard, onCopyPrompt: copyAssetPromptCard, onUploadAsset: uploadAssetFromBoard, onApproveAsset: approveAssetFromBoard, onRejectAsset: rejectAssetFromBoard, onRegisterAsset: registerAssetFromBoard, onRemoveArtifact: removeUploadedAssetFromBoard, onOpenAssetProduction: openAssetProductionShortcut });
  ...
  const buildAssetBoardFlow = (envelope: AssetBoardEnvelope, library: AssetLibraryEnvelope, shots: StoryShot[]) => {
    const boardNodes = assetBoardToFlowNodes(envelope.board, library.assets, assetBoardFilter, assetBoardShowShots, shots, { preset: assetBoardLayoutPreset, columnWidth: assetBoardColumnWidth, gap: assetBoardGap, layoutMode: assetBoardLayoutMode, collapsedScopes: assetBoardCollapsedScopes, onToggleScope: toggleAssetBoardScope, onContextMenu: openAssetContextMenu, onApprovePrompt: approveAssetPromptCard, onGeneratePrompt: generatePromptFromBoard, onGenerateFusionPrompt: generateFusionPromptFromBoard, onCopyPrompt: copyAssetPromptCard, onUploadAsset: uploadAssetFromBoard, onApproveAsset: approveAssetFromBoard, onRejectAsset: rejectAssetFromBoard, onRegisterAsset: registerAssetFromBoard, onRemoveArtifact: removeUploadedAssetFromBoard, onOpenAssetProduction: openAssetProductionShortcut });
  ...
  const restoreAssetBoardSnapshot = useCallback((snapshot: AssetBoardEditorSnapshot) => {
    ...
    const nextNodes = assetBoardToFlowNodes(board, assetLibrary?.assets || [], assetBoardFilter, assetBoardShowShots, story?.story.shots || [], { preset, columnWidth, gap, layoutMode, collapsedScopes: assetBoardCollapsedScopes, onlyBlocked: assetBoardOnlyBlocked, showCandidates: assetBoardShowCandidates, shotId: assetBoardShotId, onToggleScope: toggleAssetBoardScope, onContextMenu: openAssetContextMenu, onApprovePrompt: approveAssetPromptCard, onCopyPrompt: copyAssetPromptCard, onUploadAsset: uploadAssetFromBoard, onApproveAsset: approveAssetFromBoard, onRejectAsset: rejectAssetFromBoard, onRegisterAsset: registerAssetFromBoard, onRemoveArtifact: removeUploadedAssetFromBoard, onOpenAssetProduction: openAssetProductionShortcut }).map((node) => selectedIds.has(node.id) ? { ...node, selected: true } : node);
  ...
  const saveAssetBoard = async (background = false, expectedRevisionOverride?: number, allowRetry = true, expectedMutationEpoch?: number): Promise<AssetBoardEnvelope | null> => {
    ...
    try {
      ...
      if (unchanged) {
        const nextNodes = assetBoardToFlowNodes(saved.board, assetLibrary?.assets || [], assetBoardFilter, assetBoardShowShots, story?.story.shots || [], { preset: layout.preset, columnWidth: layout.columnWidth, gap: layout.gap, layoutMode: layout.mode, collapsedScopes: assetBoardCollapsedScopes, onToggleScope: toggleAssetBoardScope, onContextMenu: openAssetContextMenu, onApprovePrompt: approveAssetPromptCard, onCopyPrompt: copyAssetPromptCard, onUploadAsset: uploadAssetFromBoard, onApproveAsset: approveAssetFromBoard, onRejectAsset: rejectAssetFromBoard, onRegisterAsset: registerAssetFromBoard, onRemoveArtifact: removeUploadedAssetFromBoard, onOpenAssetProduction: openAssetProductionShortcut });
  ...
  const generateAssetPrompts = async (targetAssetId?: string, options: { reviewFeedback?: string; sourceQaRunId?: string; operatorIdea?: string; storyOverride?: StoryEnvelope; assetIntentVersion?: number; templateOverride?: string } = {}) => {
  ...
  const copyPromptToClipboard = async (asset: LibraryAsset, prompt: string) => {
    const fullPrompt = asset.assetClass === 'audio'
      ? composeAudioPrompt(asset, story, prompt)
      : String(asset.copyablePrompt || '').trim();
    try {
      await navigator.clipboard.writeText(fullPrompt);
    } catch {
  ...
  const importAssetCandidate = async (asset: LibraryAsset, file: File) => {
    ...
    try {
      ...
      form.append('source_type', 'external-workbench');
  ...
  const applyAssetGridLayout = useCallback((preset: AssetGridPreset = assetBoardLayoutPreset, overrides: { columnWidth?: number; gap?: number; layoutMode?: AssetBoardLayoutMode } = {}) => {
    ...
    const nextNodes = assetBoardToFlowNodes(board, assetLibrary?.assets || [], assetBoardFilter, assetBoardShowShots, story?.story.shots || [], { forceGrid: true, preset, columnWidth: nextColumnWidth, columnWidths: nextColumnWidths, gap: nextGap, layoutMode: nextLayoutMode, collapsedScopes: assetBoardCollapsedScopes, onToggleScope: toggleAssetBoardScope, onContextMenu: openAssetContextMenu, onApprovePrompt: approveAssetPromptCard, onCopyPrompt: copyAssetPromptCard, onUploadAsset: uploadAssetFromBoard, onApproveAsset: approveAssetFromBoard, onRejectAsset: rejectAssetFromBoard, onRegisterAsset: registerAssetFromBoard, onRemoveArtifact: removeUploadedAssetFromBoard, onOpenAssetProduction: openAssetProductionShortcut });
