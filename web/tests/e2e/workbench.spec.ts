...
test.describe('FrameFlow V4 workbench', () => {
  ...
  test('asset prompt actions execute and operator input can be opened and closed', async ({ page }) => {
    ...
    await expect.poll(async () => page.evaluate(() => navigator.clipboard.readText())).toContain('横向空环境参考板');
    await expect.poll(async () => page.evaluate(() => navigator.clipboard.readText())).not.toContain('Image Execution Prompt');
