...
def story_spec(document: dict[str, Any]) -> dict[str, Any]:
    ...
    rule_profile = story_rule_profile_from_spec(current)
    ...
    return {
        ...
        "rule_modules": rule_profile["modules"],
        "rule_sources": rule_profile["sources"],
        **budget,
    }
...
def story_checks(document: dict[str, Any]) -> dict[str, Any]:
    ...
    rule_profile = story_rule_profile_from_spec(payload["spec"])
    director_rules_enabled = bool(rule_profile.get("enabled"))

    def director_value_missing(value: Any) -> bool:
        if value in (None, "", []):
            return True
        ...
        return not normalized or normalized in {"待确认", "待补充", "待定", "未知", "unknown", "tbd", "todo", "未填写"}
    ...
    for scene in scenes:
        ...
        if missing or invalid_lists:
            ...
            issue("scene_ledger_incomplete", "error" if director_rules_enabled else "warning", f"场景 {scene_id} 的场景账本尚未完整，AI 工作流或人工补充后才能作为完整前期规格使用。", details=details)
        if director_rules_enabled:
            missing_director_fields = [field for field in SCENE_DIRECTOR_FIELDS if director_value_missing(scene.get(field))]
            if missing_director_fields:
                issue(
                    "scene_director_contract_incomplete",
                    "error",
                    f"场景 {scene_id} 缺少导演规则字段：{'、'.join(missing_director_fields)}。",
                    details={"missing_fields": missing_director_fields, "rule_profile": rule_profile["id"]},
                )
    total_duration = 0.0
    ...
    for shot in shots:
        ...
        if scene_ids and scene_id not in scene_ids:
            issue("scene_reference_missing", "error" if director_rules_enabled else "warning", f"镜头引用的场次 {scene_id} 未登记。", shot_id)
        for field in SHOT_DETAIL_FIELDS:
            if shot.get(field) in (None, "", []):
                issue("shot_detail_missing", "warning", f"镜头缺少连续性/生成细节 {field}。", shot_id, {"field": field})
        if shot.get("visibleEvent") in (None, "", []) or (director_rules_enabled and director_value_missing(shot.get("visibleEvent"))):
            issue("visible_event_missing", "error" if director_rules_enabled else "warning", "镜头缺少一个明确的主可见事件。", shot_id)
        if shot.get("eventConsequence") in (None, "", []) or (director_rules_enabled and director_value_missing(shot.get("eventConsequence"))):
            issue("event_consequence_missing", "error" if director_rules_enabled else "warning", "镜头缺少动作、接触、材质、光线或空间的可见后果。", shot_id)
        seedance_plan = shot.get("seedancePlan") or shot.get("seedance_plan")
        missing_seedance_fields = []
        if not isinstance(seedance_plan, dict):
            missing_seedance_fields = list(SEEDANCE_REQUIRED_FIELDS)
        elif director_rules_enabled:
            for field in SEEDANCE_REQUIRED_FIELDS:
                if field == "targetDuration":
                    try:
                        valid = float(seedance_plan.get(field) or 0) > 0
                    except (TypeError, ValueError):
                        valid = False
                    if not valid:
                        missing_seedance_fields.append(field)
                elif director_value_missing(seedance_plan.get(field)):
                    missing_seedance_fields.append(field)
        elif not seedance_plan.get("model") or not seedance_plan.get("generationMode"):
            missing_seedance_fields = ["model", "generationMode"]
        if missing_seedance_fields:
            issue(
                "seedance_plan_missing",
                "error" if director_rules_enabled else "warning",
                "镜头缺少完整 Seedance 生成计划。",
                shot_id,
                {"missing_fields": missing_seedance_fields},
            )
        continuity = shot.get("continuity")
        missing_continuity_fields = []
        if not isinstance(continuity, dict):
            missing_continuity_fields = list(CONTINUITY_FIELDS)
        elif director_rules_enabled:
            missing_continuity_fields = [field for field in CONTINUITY_FIELDS if director_value_missing(continuity.get(field) or shot.get(field))]
        elif not any(continuity.get(field) for field in ("cutIn", "cutOut", "firstFrame", "lastFrame", "editBridge")):
            missing_continuity_fields = ["cutIn", "cutOut", "firstFrame", "lastFrame", "editBridge"]
        if missing_continuity_fields:
            issue(
                "continuity_incomplete",
                "error" if director_rules_enabled else "warning",
                "镜头缺少可审阅的剪辑进出点或首尾帧连续性。",
                shot_id,
                {"missing_fields": missing_continuity_fields},
            )
        if director_rules_enabled:
            missing_director_fields = [field for field in SHOT_DIRECTOR_FIELDS if director_value_missing(shot.get(field))]
            camera_execution = shot.get("cameraExecution")
            if director_value_missing(camera_execution):
                missing_director_fields.append("cameraExecution")
            if missing_director_fields:
                issue(
                    "shot_director_contract_incomplete",
                    "error",
                    f"镜头缺少导演规则字段：{'、'.join(missing_director_fields)}。",
                    shot_id,
                    {"missing_fields": missing_director_fields, "rule_profile": rule_profile["id"]},
                )
        ...
        if not requirements:
            issue("asset_requirements_missing", "error" if director_rules_enabled else "warning", "镜头尚未登记角色、场景或道具依赖。", shot_id)
