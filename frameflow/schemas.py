...
ProviderType = Literal["jimeng_cli", "opencode", "comfyui", "minimax"]
TaskStatus = Literal[
...
class ProjectDocument(StrictModel):
    ...
    storySpec: dict[str, Any] = Field(default_factory=dict)
...
class AssetPromptRunCreate(StrictModel):
    ...
    template_override: str | None = Field(default=None, max_length=120)
...
class AssetImageGenerate(StrictModel):
    ...
    # Retained only so the removed-image-generation compatibility route can
    # validate legacy request bodies. No active execution path reads this
    # provider-size field; new asset generation uses the logical aspect ratio
    # and external generation/upload flow.
...
class StorySpecV4(StrictModel):
    ...
    # Explicit opt-in keeps historical projects on their saved story rules.
    # New projects receive frameflow-director-v2 during project creation.
    rule_profile: str = Field(default="legacy", max_length=120)
    rule_profile_version: str | None = Field(default=None, max_length=40)
    rule_modules: dict[str, list[str]] = Field(default_factory=dict)
