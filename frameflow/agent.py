...
def _assistant_asset_context(asset: dict[str, Any], prompt_chars: int = 420) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        ...
        "promptPack": {
            "templateSelection": _assistant_small_dict(pack.get("templateSelection"), 520),
            "subjectTask": _assistant_clip_text(pack.get("subjectTask"), 300),
            "compositionLayout": _assistant_clip_text(pack.get("compositionLayout"), 520),
            "visualStyleMaterials": _assistant_clip_text(pack.get("visualStyleMaterials"), 520),
            "textLabels": _assistant_clip_text(pack.get("textLabels"), 220),
            "aspectRatioOutput": _assistant_small_dict(pack.get("aspectRatioOutput"), 180),
            "constraintsNegative": _assistant_compact_scalar_list(pack.get("constraintsNegative"), 8, 180),
            "referenceRoles": _assistant_compact_scalar_list(pack.get("referenceRoles"), 8, 180),
            "actionContinuity": _assistant_clip_text(pack.get("actionContinuity"), 360),
        },
        "promptQuality": {
    ...
    for field, kind, content in legacy:
        if content not in (None, ""):
            if field == "imagePrompt":
                # ``imagePrompt`` is a retired legacy shape. Keep the
                # compatibility envelope auditable, but never turn its text
                # into a new visual Prompt or a generic fallback candidate.
                patch["unsupported_operations"].append(field)
            else:
