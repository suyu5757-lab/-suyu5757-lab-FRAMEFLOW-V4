...
from .story_rules import CONTINUITY_FIELDS, SCENE_DIRECTOR_FIELDS, SHOT_DIRECTOR_FIELDS, director_rule_profile
from .workflows import WORKFLOWS, workflow_manifest


BUNDLE_VERSION = "1.0"
STORY_CONTRACT_VERSION = "2.0"
...
STORY_VALIDATION_RULES = [
    ...
    "shot_unit_contract",
    "auto_review_gates",
    "downstream_handoff_compatibility",
]
...
def contract_bundle() -> dict[str, Any]:
    ...
    bundle: dict[str, Any] = {
        ...
        "story_contract": {
            ...
            "rule_profiles": [director_rule_profile(), {"id": "legacy", "version": None}],
            "required_shot_fields": list(SHOT_REQUIRED_FIELDS),
            "detail_fields": list(SHOT_DETAIL_FIELDS),
            "director_scene_fields": list(SCENE_DIRECTOR_FIELDS),
            "director_shot_fields": list(SHOT_DIRECTOR_FIELDS),
            "continuity_contract_fields": list(CONTINUITY_FIELDS),
...
def contract_snapshot(bundle: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "story_rule_profile": director_rule_profile()["id"],
        "story_rule_profile_version": director_rule_profile()["version"],
...
def assistant_system_instructions(
    ...
    instructions = (
        ...
        f"Prompt Contract v{prompt['version']} / {prompt['workflow']}。"
        f"Story Contract v{story['version']}；镜头必填字段：{','.join(story['required_shot_fields'])}；"
        f"连续性字段：{','.join(story['detail_fields'])}。"
        f"新项目若启用 {director_rule_profile()['id']} v{director_rule_profile()['version']}，还必须通过导演规则自动审查和下游交接兼容性检查；"
