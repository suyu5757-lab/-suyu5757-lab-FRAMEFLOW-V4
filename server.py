...
from frameflow.asset_geometry import asset_generation_profile, asset_layout_profile, normalize_aspect_ratio
...
ALLOWED_ORCHESTRATOR_MODELS={item["id"] for item in ORCHESTRATOR_MODEL_OPTIONS}; DEFAULT_ORCHESTRATOR_MODEL="opencode-go/gpt-5.6-luna"; DEFAULT_VOICE_ASSISTANT_MODEL=DEFAULT_ORCHESTRATOR_MODEL
...
SEEDED_PROVIDER_IDS=frozenset({"jimeng-default","opencode-default","minimax-default"})
...
def seed_defaults(database:Database)->None:
    ...
    with database.connect() as c:
        ...
        # Retire legacy OpenAI/OpenAI-compatible profiles in place so history
        # remains auditable while Settings, routing and execution can never use
        # them again.
        c.execute("UPDATE provider_profiles SET enabled=0,updated_at=? WHERE provider_type IN ('openai','openai_compatible')",(now,))
        c.execute("DELETE FROM capability_bindings WHERE provider_profile_id IN (SELECT id FROM provider_profiles WHERE provider_type IN ('openai','openai_compatible'))")
...
def get_profile(database:Database,pid:str)->dict[str,Any]:
    ...
    if str(row["provider_type"]) in LEGACY_PROVIDER_TYPES:raise HTTPException(410,"OpenAI Provider 已从工作台移除。")
...
def validate_orchestrator_model(profile:dict[str,Any],model:str|None)->None:
    ...
    if profile["provider_type"] != "opencode":
        raise HTTPException(422,"编排能力仅允许绑定 OpenCode。")
...
async def list_profiles(request:Request):
    database=db(request)
    with database.connect() as c:rows=c.execute("SELECT * FROM provider_profiles WHERE provider_type NOT IN ('openai','openai_compatible') ORDER BY created_at").fetchall()
...
async def update_profile(pid:str,body:ProviderProfileUpdate,request:Request):
    ...
    if current["provider_type"] in LEGACY_PROVIDER_TYPES:raise HTTPException(410,"OpenAI Provider 已从工作台移除。")
...
async def create_project_v3(body:ProjectCreateV3,request:Request):
    ...
    with database.connect() as connection:
        ...
        document = {
            ...
            "storyboardVersions": [],
            "storyWorkflowRuns": [],
            # New projects opt into the director rule bundle. Existing project
            # documents intentionally keep their prior behaviour because they
            # do not receive this field through a read path or migration.
            "storySpec": {
                "workflow_mode": "optimize_script_and_storyboard",
                "creative_goal": body.brief.strip(),
                "duration": body.duration,
                "reference_duration": body.duration,
                "ratio": body.ratio,
                "generator_profile": body.generator,
                "rule_profile": DIRECTOR_RULE_PROFILE,
                "rule_profile_version": director_rule_profile()["version"],
                "rule_modules": {
                    key: list(value)
                    for key, value in director_rule_profile().items()
                    if key in {"scriptModules", "storyboardModules", "handoffModules"}
                },
                "rule_sources": director_rule_profile()["sources"],
            },
        }
    try:
...
def _assistant_director_review(document:dict[str,Any],content:dict[str,Any]|None=None)->dict[str,Any]|None:
    """Run the same director review for assistant story candidates on v2 projects."""
    spec=document.get("storySpec") if isinstance(document.get("storySpec"),dict) else {}
    profile=spec.get("rule_profile") or spec.get("ruleProfile")
    if not is_director_rule_profile(profile):
        return None
    active_script=next((item for item in document.get("scriptVersions",[]) if isinstance(item,dict) and item.get("status")=="active"),{})
    source_script=str(document.get("script") or "")
    source_hash=hashlib.sha256(source_script.encode("utf-8")).hexdigest()
    candidate_content=content if isinstance(content,dict) else {}
    candidate={
        "proposedScript":source_script,
        "sourceScriptHash":source_hash,
        "structure":spec.get("structure") or [],
        "beats":spec.get("beats") or [],
        "productionElements":candidate_content.get("productionElements") if isinstance(candidate_content.get("productionElements"),dict) else {},
        "scenes":document.get("scenes") or [],
        "shots":document.get("shots") or [],
        "assetHandoff":candidate_content.get("assetHandoff") if isinstance(candidate_content.get("assetHandoff"),dict) else document.get("assetHandoff") if isinstance(document.get("assetHandoff"),dict) else {},
        "sourceBeatCoverage":candidate_content.get("sourceBeatCoverage") if isinstance(candidate_content.get("sourceBeatCoverage"),dict) else {},
    }
    ledger=build_source_beat_ledger(source_script)
    if not candidate["sourceBeatCoverage"]:
        candidate["sourceBeatCoverage"]=storyboard_source_coverage(source_script,candidate,ledger=ledger)
    package={
        "workflow_mode":str(spec.get("workflow_mode") or "optimize_script_and_storyboard"),
        "current_script":source_script,
        "source_script_version_id":active_script.get("id"),
        "source_script_hash":source_hash,
        "source_beat_ledger":ledger,
        "existing_asset_ids":[str(item.get("id")) for item in document.get("assets",[]) if isinstance(item,dict) and item.get("id")],
        "rule_profile":profile,
        "rule_profile_version":spec.get("rule_profile_version") or spec.get("ruleProfileVersion"),
        "rule_modules":spec.get("rule_modules") or spec.get("ruleModules") or {},
        "rule_sources":spec.get("rule_sources") or spec.get("ruleSources") or director_rule_profile()["sources"],
    }
    return director_auto_review(candidate,package,repair_attempts=0)
...
def _assistant_normalize_operation_content(operation: dict[str, Any], doc: dict[str, Any], bundle: dict[str, Any], attachment_ids: list[str]) -> dict[str, Any]:
    ...
    if workspace == "assets" and isinstance(content, dict):
        ...
        source_spec=doc.get("storySpec") if isinstance(doc.get("storySpec"),dict) else {}
        if is_director_rule_profile(source_spec) and not is_director_rule_profile(proposed.get("storySpec")):
            proposed["storySpec"]={
                **(proposed.get("storySpec") if isinstance(proposed.get("storySpec"),dict) else {}),
                "rule_profile":source_spec.get("rule_profile") or DIRECTOR_RULE_PROFILE,
                "rule_profile_version":source_spec.get("rule_profile_version") or director_rule_profile()["version"],
                "rule_modules":deepcopy(source_spec.get("rule_modules") or source_spec.get("ruleModules") or {}),
        ...
        director_review=_assistant_director_review(proposed,proposed_content)
        if isinstance(director_review,dict) and director_review.get("acceptanceAllowed") is False:
            raise ProviderError("Agent 剧本/分镜候选未通过 frameflow-director-v2 自动审查：" + "；".join(str(item.get("message") or item.get("code") or "") for item in director_review.get("issues",[]) if isinstance(item,dict) and item.get("severity")=="blocking"), "validation", 502)
        if director_review is not None:
            content={**content,"autoReview":director_review}
            result["content"]=content
            result["after"]=content
...
def _assistant_story_candidate(doc: dict[str, Any], content: Any) -> dict[str, Any]:
    ...
    director_review=_assistant_director_review(proposed,payload if isinstance(payload,dict) else {})
    if isinstance(director_review,dict) and director_review.get("acceptanceAllowed") is False:
        raise HTTPException(422,{"message":"故事候选未通过 frameflow-director-v2 自动审查。","review":director_review})
    checks = story_checks(proposed)
    if checks.get("errors", 0):
...
def _assistant_asset_prompt_candidate(database: Database, connection: sqlite3.Connection, doc: dict[str, Any], operation: Any, bundle: dict[str, Any]) -> tuple[dict[str, Any], str | None]:
    ...
    asset.update({
        ...
        "mustPreserve": canonical.get("mustPreserve") or asset.get("mustPreserve") or [],
        "mustAvoid": canonical.get("mustAvoid") or asset.get("mustAvoid") or [],
    })
...
def _settings_providers(database: Database) -> list[dict[str, Any]]:
    with database.connect() as connection:
        rows = connection.execute("SELECT * FROM provider_profiles WHERE provider_type NOT IN ('openai','openai_compatible') ORDER BY display_name, id").fetchall()
...
async def write_story_v3(project_id:str,body:StoryDocumentUpdateV3,request:Request):
    ...
    previous_spec=doc.get("storySpec") if isinstance(doc.get("storySpec"),dict) else {}
    incoming_spec=payload.get("spec") if isinstance(payload.get("spec"),dict) else {}
    if is_director_rule_profile(previous_spec) and not is_director_rule_profile(incoming_spec):
        # A legacy caller may omit newly added profile fields. Preserve the
        # explicit rule bundle of a new project instead of downgrading it just
        # because Pydantic filled the omitted fields with legacy defaults.
        incoming_spec={
            **incoming_spec,
            "rule_profile":previous_spec.get("rule_profile") or DIRECTOR_RULE_PROFILE,
            "rule_profile_version":previous_spec.get("rule_profile_version") or director_rule_profile()["version"],
            "rule_modules":deepcopy(previous_spec.get("rule_modules") or previous_spec.get("ruleModules") or {}),
            "rule_sources":deepcopy(previous_spec.get("rule_sources") or previous_spec.get("ruleSources") or director_rule_profile()["sources"]),
        }
        payload["spec"]=incoming_spec
...
def _storyboard_input_package(doc:dict[str,Any],body:StoryOptimizationCreate)->dict[str,Any]:
    ...
    default_rule_profile=director_rule_profile()
    rule_profile_id=str(current_spec.get("rule_profile") or current_spec.get("ruleProfile") or "legacy")
    director_rules_enabled=is_director_rule_profile(rule_profile_id)
    configured_modules=current_spec.get("rule_modules") or current_spec.get("ruleModules")
    rule_modules=configured_modules if director_rules_enabled and isinstance(configured_modules,dict) else {
        key:list(value)
        for key,value in default_rule_profile.items()
        if key in {"scriptModules","storyboardModules","handoffModules"}
    } if director_rules_enabled else {}
    configured_sources=current_spec.get("rule_sources") or current_spec.get("ruleSources")
    rule_sources=configured_sources if director_rules_enabled and isinstance(configured_sources,list) and configured_sources else default_rule_profile.get("sources",[]) if director_rules_enabled else []
    rule_profile_version=(
        str(current_spec.get("rule_profile_version") or current_spec.get("ruleProfileVersion") or default_rule_profile["version"])
        if director_rules_enabled else None
    )
    ...
    return {
        ...
        "rule_profile":DIRECTOR_RULE_PROFILE if director_rules_enabled else "legacy",
...
def _normalise_storyboard_handoff(result:dict[str,Any],input_package:dict[str,Any]|None=None)->dict[str,Any]:
    ...
    scene_aliases={
        ...
        "sceneGoal":("sceneGoal","scene_goal","goal","场景目标"),
        "characterDesire":("characterDesire","character_desire","desire","人物欲望"),
        "primaryObstacle":("primaryObstacle","primary_obstacle","obstacle","主要阻碍"),
        "observableAction":("observableAction","observable_action","visibleAction","可观察行动"),
        "stateChange":("stateChange","state_change","change","状态变化"),
        "turn":("turn","sceneTurn","scene_turn","转折"),
        "entryState":("entryState","entry_state","进入状态"),
        "exitState":("exitState","exit_state","退出状态"),
    }
    ...
    shot_aliases={
        ...
        "subjectFocus":("subjectFocus","subject_focus","focus","主体焦点"),
        "performance":("performance","expression","acting","表演","表情"),
        "blocking":("blocking","blockingPlan","blocking_plan","调度","走位"),
        "coverageRole":("coverageRole","coverage_role","coverage","coverage职责","镜头覆盖职责"),
        "environmentPressure":("environmentPressure","environment_pressure","环境压力"),
        "microAction":("microAction","micro_action","身体微动作"),
        "visualMotif":("visualMotif","visual_motif","soundMotif","sound_motif","视觉母题","声音母题"),
    }
    ...
    for index,raw in enumerate(raw_shots):
        ...
        for key,default in {
            ...
            "blocking":"待确认",
            "coverageRole":"待确认",
            "environmentPressure":"待确认",
            "microAction":"待确认",
            "visualMotif":"待确认",
        }.items():
            raw_value=_storyboard_first_value(shot,shot_aliases[key])
            if raw_value in (None,"",[]):
                notes.append(f"{shot_id} 的 {key} 使用了待确认默认值。")
                note_default(f"shots[{index}]",key)
            shot[key]=_storyboard_scalar(shot,shot_aliases[key],default)

        raw_camera_execution=_storyboard_first_value(shot,("cameraExecution","camera_execution","cameraPlan","camera_plan","摄影机执行","摄影执行"))
        if isinstance(raw_camera_execution,dict):
            shot["cameraExecution"]=dict(raw_camera_execution)
        else:
            if raw_camera_execution in (None,"",[]):
                notes.append(f"{shot_id} 的 cameraExecution 使用了待确认默认值。")
                note_default(f"shots[{index}]","cameraExecution")
            shot["cameraExecution"]={"instruction":_storyboard_scalar(shot,("cameraExecution","camera_execution","cameraPlan","camera_plan","摄影机执行","摄影执行"),"待确认")}
        ...
        raw_requirements=_storyboard_list(_storyboard_first_value(shot,("assetRequirements","asset_requirements","资产需求")))
        requirements=[]
        for requirement in raw_requirements:
            if not isinstance(requirement,dict):
                continue
            normalized_requirement=dict(requirement)
            asset_id=_storyboard_scalar(normalized_requirement,("assetId","asset_id","id","资产ID"))
            asset_class=_storyboard_scalar(normalized_requirement,("assetClass","asset_class","class","资产类型"))
            normalized_requirement["shotId"]=_storyboard_scalar(normalized_requirement,("shotId","shot_id","镜头ID"),shot_id)
            if asset_id:
                normalized_requirement["assetId"]=asset_id
            if asset_class:
                normalized_requirement["assetClass"]=asset_class
            normalized_requirement.setdefault("role",_storyboard_scalar(normalized_requirement,("role","productionRole","production_role","职责"),""))
            normalized_requirement.setdefault("priority",_storyboard_scalar(normalized_requirement,("priority","grade","优先级"),"B"))
            normalized_requirement.setdefault("required",True)
            normalized_requirement.setdefault("requiredReadiness",_storyboard_scalar(normalized_requirement,("requiredReadiness","required_readiness","就绪要求"),"production"))
...
def _canonical_story_shot(shot:dict[str,Any],project_output_aspect_ratio:Any="9:16")->dict[str,Any]:
    """Attach the style-library visual Prompt contract to an accepted shot."""
    source=dict(shot)
    raw_pack=source.get("promptPack") or source.get("prompt_pack") or {}
    pack=dict(raw_pack) if isinstance(raw_pack,dict) else {}
    pack.setdefault("subjectTask",source.get("subjectTask") or source.get("purpose") or "为本镜头建立可执行的视觉事件")
    pack.setdefault("compositionLayout",source.get("compositionLayout") or source.get("composition") or source.get("camera") or source.get("size") or "")
    pack.setdefault("visualStyleMaterials",source.get("visualStyleMaterials") or source.get("styleDescription") or source.get("style") or "")
    pack.setdefault("textLabels",source.get("textLabels") or "")
    pack.setdefault("actionContinuity",source.get("actionContinuity") or source.get("action") or source.get("continuity") or "")
    pack.setdefault("constraintsNegative",source.get("constraintsNegative") or source.get("mustAvoid") or [])
    ...
    canonical=canonicalize_prompt_output(
        ...
        context={"shots":[source],"references":source.get("referenceRoles") or [],"output_aspect_ratio":project_output_aspect_ratio},
    )
    ...
    seedance_plan={
        ...
        "aspectRatio":normalize_aspect_ratio(project_output_aspect_ratio,"9:16"),
    ...
    return {**source,**canonical,"seedancePlan":seedance_plan,"continuity":continuity,"mustPreserve":canonical.get("mustPreserve") or [],"mustAvoid":canonical.get("mustAvoid") or []}


def _normalise_story_asset_requirement(value:Any,shot_id:str)->dict[str,Any]|None:
    """Keep storyboard requirements on stable asset/shot IDs.

    This is an adapter between the new storyboard contract and the existing
    regulator/asset-intent flow.  It never creates a logical asset and leaves
    Prompt semantics untouched.
    """
    if not isinstance(value,dict):
        return None
    result=dict(value)
    asset_id=str(result.get("assetId") or result.get("asset_id") or "").strip()
    if not asset_id:
        return None
    result["assetId"]=asset_id
    result["shotId"]=str(shot_id)
    if result.get("assetClass") in (None,"") and result.get("asset_class") not in (None,""):
        result["assetClass"]=result.get("asset_class")
    result.setdefault("assetClass","unknown")
    result.setdefault("role","")
    result.setdefault("priority","B")
    result.setdefault("required",True)
    result.setdefault("requiredReadiness","production")
    return result


def _merge_story_asset_requirements(existing:Any,candidate:Any,shot_id:str)->list[dict[str,Any]]:
    """Merge regulator enrichment without dropping accepted storyboard intent."""
    merged:dict[str,dict[str,Any]]={}
    order:list[str]=[]
    for raw in [*_storyboard_list(existing),*_storyboard_list(candidate)]:
        requirement=_normalise_story_asset_requirement(raw,shot_id)
        if requirement is None:
            continue
        asset_id=str(requirement["assetId"])
        if asset_id not in merged:
            merged[asset_id]=requirement
            order.append(asset_id)
            continue
        current=merged[asset_id]
        for key,value in requirement.items():
            if key == "shotId":
                current[key]=str(shot_id)
            elif value not in (None,"",[],{}):
                current[key]=deepcopy(value)
    return [merged[asset_id] for asset_id in order]


def _storyboard_handoff_provenance(doc:dict[str,Any],storyboard_version_id:str|None=None)->dict[str,Any]:
    """Build read-only lineage metadata for asset intent and Prompt inputs."""
    active_script=next((item for item in doc.get("scriptVersions",[]) if isinstance(item,dict) and item.get("status")=="active"),{})
    active_storyboard=next((item for item in doc.get("storyboardVersions",[]) if isinstance(item,dict) and item.get("status")=="active"),{})
    # Once a storyboard is accepted, its provenance is the authority for all
    # downstream handoffs. Regulator enrichment and Prompt preparation may add
    # metadata to the live shot objects, but those changes are not a new
    # storyboard version and must not silently change its source fingerprint.
    if storyboard_version_id is None and isinstance(active_storyboard.get("provenance"),dict):
        stored=active_storyboard["provenance"]
        if stored.get("sourceFingerprint") and stored.get("sourceStoryboardVersionId"):
            return {
                "sourceScriptVersionId":stored.get("sourceScriptVersionId") or active_script.get("id"),
                "sourceStoryboardVersionId":stored.get("sourceStoryboardVersionId") or active_storyboard.get("id"),
                "sourceShotIds":[str(value) for value in stored.get("sourceShotIds",[]) if value],
                "sourceFingerprint":str(stored["sourceFingerprint"]),
            }
    shot_ids=[str(item.get("id")) for item in doc.get("shots",[]) if isinstance(item,dict) and item.get("id")]
    payload={
    ...
    return {
        "sourceScriptVersionId":active_script.get("id"),
        "sourceStoryboardVersionId":storyboard_version_id or active_storyboard.get("id"),
        "sourceShotIds":shot_ids,
        "sourceFingerprint":f"story:{hashlib.sha256(encoded.encode('utf-8')).hexdigest()}",
    }

def _require_opencode_prompt_provider(profile:dict[str,Any])->None:
    """All planning and visual Prompt agents use OpenCode exclusively."""
    if str(profile.get("provider_type") or "") != "opencode":
        raise HTTPException(409,"当前资产与 Prompt Agent 仅使用 OpenCode，请在设置中配置并绑定 OpenCode。")
...
async def _run_storyboard_agent(request:Request,project_id:str,input_package:dict[str,Any])->dict[str,Any]:
    ...
    director_rule=(
        "当前项目启用 frameflow-director-v2。先在 structure / beats 中写清故事目标、人物欲望、冲突、行动、风险、转折和结尾；"
        "不得用抽象说明代替可观察的行为。每个 scene 除基础 Scene Ledger 外，必须填写 sceneGoal、characterDesire、primaryObstacle、observableAction、stateChange、turn、entryState、exitState。"
        "每个镜头必须同时填写 subjectFocus、performance、blocking、coverageRole、environmentPressure、microAction、visualMotif、cameraExecution、spatialGeography、materialEvidence、lightingCausality。"
        "blocking 先说明人物/道具在空间中的走位与相互关系，再说明摄影机；coverageRole 必须说明这个镜头是建立空间、交代关系、展示行动、展示反应、提供信息、连接剪辑或强化节奏中的哪一种。"
        "每个镜头只能有一个主可见事件、一个可播放变化和一个可编辑 clip unit；不要把多个独立剪辑事件伪装成一个长镜头。"
        "每个镜头的 assetRequirements 必须是对象数组，每项都要有 shotId（等于当前镜头 ID）、assetId、assetClass、role、priority、required、requiredReadiness；每个 assetId 必须在 assetHandoff 的稳定资产清单中声明。"
        "assetHandoff 必须包含 characters、scenes、props、soundRequirements、shotAssetMatrix；generationReferenceAssets 的每一项必须给出 assetId、role、controls、doesNotControl、required、reason 和 relevantShots，不能引用自身或未声明的 ID。"
        "不要因为缺少事实而编造；任何不直接来自 sourceBeatLedger 的新增情节、人物、道具或设定，都必须在 assumptions、creativeAdditions 或 needsConfirmation 中逐项标明来源、理由和确认状态，并在对应记录保留 origin；director 规则字段不能只写‘待确认’。"
        if is_director_rule_profile(input_package.get("rule_profile")) else "当前项目使用既有剧本与分镜规则。"
    )
    ...
    _require_opencode_prompt_provider(profile)
...
# Live visual Prompt agents.  They intentionally share one rule source: the
# versioned gpt-image-2-style-library compiler and its minimal Prompt Pack.
async def _run_asset_prompt_agent(request:Request,project_id:str,input_package:dict[str,Any])->dict[str,Any]:
    ...
    validate_orchestrator_model(profile,model); _require_opencode_prompt_provider(profile)
    instructions=(
        "你是视觉资产 Prompt 编排器。视觉 Prompt 的唯一主要规则来源是 gpt-image-2-style-library；"
        "按输出目标、模板类别、visual style、scene tag、近似案例和模板 pitfalls 选择正式模板。"
        "只返回结构化 JSON，不生成图片，不调用图片服务。每个非融合视觉资产必须返回 id、assetClass、name、priority、required、targetSkill、relevantShots、prompt、promptPack。"
        "promptPack 只能使用 templateSelection、subjectTask、compositionLayout、visualStyleMaterials、textLabels、aspectRatioOutput、constraintsNegative、referenceRoles；融合和镜头任务再使用 actionContinuity。"
        "不要返回旧版 promptIntent、identityAnchor、visibleEvent、characterDetails、sceneDetails、propDetails、fusionDetails、shotPlan、visualStyle、mustPreserve、mustAvoid、generationNotes、suggestedSize 或 provider 尺寸字段。"
        "prompt 字段只写面向图像生成的自然语言内容；不要写模板 ID、案例编号、规则来源、QA、工作流、资产 ID、Provider、OpenCode、OpenAI、JSON、上传或保存指令。"
        "FrameFlow 只保留资产固定画面比例、图像输出格式、项目比例继承和角色横向四区参考板这几项生产硬约束。"
        "角色资产必须是横向四区：左侧约42%面部与上半身特写，右侧约58%依次为正面、严格90°侧面、严格180°背面全身；四区属于同一角色。"
        "场景默认16:9空环境，独立道具和产品默认1:1，融合与镜头继承项目比例；图像输出为PNG。"
        "如果输入包含 operator_idea 或 review_feedback，将其整合为视觉内容而不是机械追加；如果输入包含旧 Prompt，只提取用户事实，不复制旧句子或旧规则。"
        "融合卡只保留连接资产的可见身份、空间关系、动作、光线、遮挡和连续性，不创建新的逻辑资产。"
        + prompt_contract_instructions()
    )
    text=f"请根据以下资产意图、分镜和已确认的资产关系生成新的视觉 Prompt 卡。\n\n输入包：{json.dumps(input_package,ensure_ascii=False)}"
...
async def _run_asset_prompt_consistency_agent(request:Request,project_id:str,input_package:dict[str,Any])->dict[str,Any]:
    ...
    validate_orchestrator_model(profile,model); _require_opencode_prompt_provider(profile)
    instructions=(
        "你是视觉资产 Prompt 一致性审核器。只审核最终 Prompt 和新的 style-library Prompt Pack，不生成图片、不执行图片 QA、不批准生成。"
        "检查模板是否来自 gpt-image-2-style-library、六个图像 Prompt 区块是否完整、资产边界是否正确、画面比例与输出格式是否正确、可复制 Prompt 是否只含图像内容、融合/镜头是否包含动作与连续性。"
        "角色必须检查左侧约42%特写与右侧正面/90°侧面/180°背面三张等宽全身视图；场景必须是空环境；道具和产品不得混入人物或场景融合。"
        "如果发现旧字段、旧规则、内部元数据、Provider 像素尺寸或流程指令，必须 needs_repair。全部通过时 decision=pass。只返回结构化 JSON。"
    )
    text=f"请审核以下新的视觉 Prompt。\n\n输入包：{json.dumps(input_package,ensure_ascii=False)}"
...
async def _run_fusion_prompt_agent(request:Request,project_id:str,input_package:dict[str,Any],provider_profile_id:str|None=None,requested_model:str|None=None)->dict[str,Any]:
    ...
    validate_orchestrator_model(profile,model); _require_opencode_prompt_provider(profile)
    instructions=(
        "你是 gpt-image-2-style-library 驱动的融合 Prompt 编排器。必须选择 scene-storytelling 模板，"
        "把已确认连接的角色、道具和场景组织成同一张可生成的画面。只返回结构化 JSON，不生成图片。"
        "必须分别保留各输入资产的可见身份、结构、材质和比例，先建立角色与道具的接触关系，再放入场景；明确空间位置、尺度链、遮挡、接触阴影、投射阴影、环境光遮蔽、材质响应、镜头动作和连续性。"
        "promptPack 只能使用新的 style-library 字段；不要使用旧版 identityAnchor、fusionDetails、shotPlan 等字段。"
        "最终 prompt 只写图像生成有帮助的自然语言，不写模板 ID、案例编号、资产 ID、QA、Provider、OpenCode、OpenAI、工作流或上传指令。"
        + prompt_contract_instructions(fusion=True)
    )
    text=f"请根据以下已确认连接生成正式融合 Prompt。\n\n输入包：{json.dumps(input_package,ensure_ascii=False)}"
...
def _merge_prompt_card_components(parent_id:str,children:list[tuple[str,dict[str,Any]]],source_asset:dict[str,Any]|None=None)->dict[str,Any]:
    ...
    if prompt_parts:
        primary["prompt"]="\n\n".join(prompt_parts)
    # Component lineage is an internal audit record, never a semantic Prompt
    # Pack block. The compiler must not render provider-created subcomponent
    # labels into the user-facing Prompt.
    primary["componentDetails"]=component_details
    primary["externalGenerationRequired"]=True
    return primary
...
def _prompt_context(doc:dict[str,Any],asset:dict[str,Any]|None=None,relevant_shots:Any=None)->dict[str,Any]:
    ...
    return {
        ...
        # ``aspect_ratio`` is the semantic asset canvas. Provider pixel sizes
        # are intentionally absent from the Prompt context.
...
def _stored_prompt_composition_mode(asset:dict[str,Any]|None, requested:str="style_library")->str:
    """Return the one active visual Prompt compiler mode."""
...
def _canonical_prompt_card(
    ...
    shots:Any=None,
    *,
    composition_mode:str="style_library",
    ...
    if asset_class in {"character","scene","prop","product","style","fusion","shot"} and not prompt_pack_has_generation_fields(asset_class,provider_prompt_pack):
        raise ValueError("AI 未返回完整的 style-library Prompt Pack，不能使用旧 Prompt 或纯文本回退。")
    ...
    source_prompt_pack=(
        ...
        if effective_mode == "style_library"
    ...
    canonical=canonicalize_prompt_output(
        ...
        template_override=card.get("templateOverride") or card.get("template_override"),
    )
    pack=canonical["promptPack"]
    cleaned_card={key:value for key,value in card.items() if key not in {
        "componentDetails", "imageGenerationEligible", "image_generation_eligible",
        "assetGenerationSize", "asset_generation_size", "assetProviderAspectRatio", "asset_provider_aspect_ratio",
        "providerAspectRatio", "provider_aspect_ratio", "imageExecutionPrompt", "image_execution_prompt",
        "generationNotes", "suggestedSize", "identityAnchor", "identity_anchor",
    }}
    return {
        **cleaned_card,
        "assetClass":asset_class,
        **canonical,
        "aspectRatio":(canonical.get("aspectRatioOutput") or {}).get("aspectRatio") if isinstance(canonical.get("aspectRatioOutput"),dict) else None,
        "outputFormat":(canonical.get("aspectRatioOutput") or {}).get("outputFormat") if isinstance(canonical.get("aspectRatioOutput"),dict) else "png",
...
def _persist_asset_prompt_card(
    ...
    *,
    ...
    asset.update({
        ...
        "mustAvoid":avoid,
        "externalGenerationRequired":cls in {"character","scene","prop","product","fusion","style","shot"},
        "uploadAllowed":cls in {"character","scene","prop","product","fusion","style","shot"},
        "imageQaRequired":cls in {"character","scene","prop","product","fusion","style","shot"},
    })
    if canonical.get("promptCompositionMode"):
        asset["promptCompositionMode"]=canonical["promptCompositionMode"]
    if canonical.get("promptCompilerVersion"):
    ...
    return {
        **canonical,
        ...
        "aspectRatio":generation_profile["aspect_ratio"],
        "outputFormat":"png",
...
def _canonical_prompt_for_asset(
    ...
    *,
    ...
    """Compile any persisted/manual visual Prompt through the style-library workflow."""
    metadata=_asset_metadata(asset)
    ...
    return canonicalize_prompt_output(
        ...
        strict_clean=strict_clean,
        preserve_clean_prompt=preserve_clean_prompt,
    )
    ...
    for shot in shots:
        ...
        if not existing_id and create:
            ...
            asset={
                ...
                "fusionPromptQaAllowed":False,"fusionPromptGenerationAllowed":False,"externalGenerationRequired":True,
                "assetMetadata":{
...
def _fusion_auto_gate(doc:dict[str,Any],asset:dict[str,Any])->dict[str,Any]:
    ...
    for source_id in source_ids:
        ...
        if not source:
            ...
            continue
        readiness=_asset_readiness_with_prompt_contract(source)
...
def _validate_asset_prompt_output(result:dict[str,Any],allowed_ids:set[str],expected_ids:set[str]|None=None,shot_ids:set[str]|None=None)->list[str]:
    ...
    for card in cards:
        ...
        if not isinstance(card.get("promptPack"),dict):
            issues.append(f"资产 {asset_id} 的 promptPack 必须是对象")
        ...
        elif card_class in {"character","scene","prop","product","style","fusion","shot"}:
            prompt_pack=card.get("promptPack")
            if not prompt_pack_has_generation_fields(card_class,prompt_pack):
                issues.append(f"资产 {asset_id} 的 Prompt Pack 必须只包含新的 style-library 视觉字段")
            else:
                selection=prompt_pack.get("templateSelection") if isinstance(prompt_pack.get("templateSelection"),dict) else {}
                template_id=str(selection.get("templateId") or "").strip()
                try:
                    select_style_template(card_class,template_override=template_id)
                except ValueError:
                    issues.append(f"资产 {asset_id} 使用了不存在的 style library 模板 {template_id or '<empty>'}")
                for boundary_issue in validate_clean_prompt(str(card.get("prompt") or ""),asset_class=card_class,base_asset=card_class in {"character","scene","prop","product"}):
                    issues.append(f"资产 {asset_id}：{boundary_issue}")
...
PROMPT_CONSISTENCY_CHECK_FIELDS=("style_library","prompt_blocks","asset_scope","geometry","copy_boundary","continuity")
...
def _local_visual_prompt_issues(asset:dict[str,Any],card:dict[str,Any],doc:dict[str,Any])->list[dict[str,str]]:
    """Run deterministic style-library, boundary and geometry checks."""
    cls=canonical_asset_class(card.get("assetClass") or _asset_class(asset) or "")
    if cls not in {"character","scene","prop","product","style","fusion","shot"}:return []
    prompt=str(card.get("prompt") or "").strip()
    pack=card.get("promptPack") if isinstance(card.get("promptPack"),dict) else {}
    issues=[]
    if not prompt:
        issues.append(_prompt_consistency_issue("prompt_missing","最终可复制 Prompt 为空。"))
    if not prompt_pack_has_generation_fields(cls,pack):
        issues.append(_prompt_consistency_issue("prompt_blocks_incomplete","Prompt Pack 未形成完整的 style-library 视觉 Prompt。"))
    selection=pack.get("templateSelection") if isinstance(pack.get("templateSelection"),dict) else {}
    template_id=str(selection.get("templateId") or "").strip()
    if not template_id:
        issues.append(_prompt_consistency_issue("style_library_missing","Prompt 未记录有效的 style library 模板。"))
    else:
        try:
            select_style_template(cls,template_override=template_id)
        except ValueError:
            issues.append(_prompt_consistency_issue("style_library_invalid","Prompt 使用了不存在的 style library 模板。"))
    profile=_asset_generation_profile(asset,doc.get("ratio") or "9:16")
    geometry_prompt=base_asset_geometry_prompt(cls,{"output_aspect_ratio":profile.get("aspect_ratio")})
    if geometry_prompt and geometry_prompt not in prompt:
        issues.append(_prompt_consistency_issue("geometry_missing","Prompt 没有包含当前资产的固定比例和布局要求。"))
    output_spec=pack.get("aspectRatioOutput") if isinstance(pack.get("aspectRatioOutput"),dict) else {}
    if str(output_spec.get("aspectRatio") or "") != str(profile.get("aspect_ratio") or ""):
        issues.append(_prompt_consistency_issue("geometry_ratio_mismatch","Prompt Pack 的资产比例与系统固定比例不一致。"))
    if str(output_spec.get("outputFormat") or "") != "png":
        issues.append(_prompt_consistency_issue("output_format_mismatch","图像资产的输出格式必须是 PNG。"))
    for boundary_issue in validate_clean_prompt(prompt,asset_class=cls,base_asset=cls in {"character","scene","prop","product"}):
        issues.append(_prompt_consistency_issue("copy_boundary",boundary_issue))
    if cls=="character":
        required_tokens=("左侧约42%","角色面部与上半身特写","右侧约58%","正面全身","严格90°侧面全身","严格180°背面全身")
        missing=[token for token in required_tokens if token not in prompt]
        if missing:
            issues.append(_prompt_consistency_issue("character_four_zone",f"角色四区布局缺少：{'、'.join(missing)}。"))
    elif cls=="scene":
        leaked_markers=("角色进入画面","人物进入画面","角色握住","人物手持","角色穿戴","物品与角色融合")
        if any(marker in prompt and not re.search(r"(?:不|无|禁止|避免|不得|不要)[^。；，,]{0,8}"+re.escape(marker),prompt) for marker in leaked_markers):
            issues.append(_prompt_consistency_issue("asset_scope","场景基础资产不能包含角色、独立道具或融合动作。"))
    elif cls in {"prop","product"}:
        leaked_markers=("角色握住","人物手持","角色穿戴","放置在场景中","物品与角色融合")
        if any(marker in prompt and not re.search(r"(?:不|无|禁止|避免|不得|不要)[^。；，,]{0,8}"+re.escape(marker),prompt) for marker in leaked_markers):
            issues.append(_prompt_consistency_issue("asset_scope","独立物件/产品资产不能包含人物或场景融合动作。"))
    elif cls in {"fusion","shot"} and not str(pack.get("actionContinuity") or "").strip():
        issues.append(_prompt_consistency_issue("continuity_missing","融合或镜头 Prompt 必须包含可见动作与连续性。"))
    unique=[]; seen=set()
    for item in issues:
        if item["code"] in seen:continue
        seen.add(item["code"]); unique.append(item)
    return unique[:8]
...
def _local_base_asset_prompt_issues(asset:dict[str,Any],card:dict[str,Any],doc:dict[str,Any])->list[dict[str,str]]:
    ...
    return _local_visual_prompt_issues(asset,card,doc)
        ...
        for asset_id in failed_ids:
            ...
            try:
                ...
                canonical=_canonical_prompt_card(
                    ...
                    composition_mode="style_library",
                    strict_clean=True,
                )
                ...
                prompt_by_id[asset_id]={**canonical,"assetClass":canonical.get("assetClass") or _asset_class(asset),"targetSkill":target_skill,"assetAspectRatio":asset.get("assetAspectRatio"),"assetOutputFormat":asset.get("assetOutputFormat"),"assetLayoutProfile":asset.get("assetLayoutProfile")}
...
async def rebase_asset_intents(project_id:str,body:AssetIntentPrepareCreate,request:Request):
    ...
    doc["assetIntent"]={"assetIntentVersion":state["assetIntentVersion"]+1,"sourceStoryRevision":_asset_intent_story_revision(doc),"sourceAssetManifestFingerprint":_asset_intent_manifest_fingerprint(doc),"sourceProvenance":_asset_intent_provenance(doc),"entries":entries,"updatedAt":now}
...
async def interpret_asset_intent(project_id:str,asset_id:str,body:AssetIntentInterpretCreate,request:Request):
    ...
    current_provenance=_asset_intent_provenance(doc)
    ...
    previous_provenance=previous_state.get("sourceProvenance") if isinstance(previous_state.get("sourceProvenance"),dict) else None
    if previous_provenance and (
        previous_provenance.get("sourceFingerprint")!=current_provenance.get("sourceFingerprint")
        or previous_provenance.get("sourceStoryboardVersionId")!=current_provenance.get("sourceStoryboardVersionId")
    ):
        raise HTTPException(409,{"message":"当前分镜版本已经变化，请重新打开资产创作意图界面确认。","storyboard_stale":True})
    ...
    source_provenance=previous_provenance or current_provenance
    ...
    try:
        ...
        latest_doc["assetIntent"]={"assetIntentVersion":latest_state["assetIntentVersion"]+1,"sourceStoryRevision":latest_state.get("sourceStoryRevision") or source_story_revision,"sourceAssetManifestFingerprint":latest_state.get("sourceAssetManifestFingerprint") or source_fingerprint,"sourceProvenance":latest_state.get("sourceProvenance") or source_provenance,"entries":list(latest_entries.values()),"updatedAt":utcnow()}
...
async def generate_asset_prompts(project_id:str,body:AssetPromptRunCreate,request:Request):
    ...
    if body.template_override:
        try:
            select_style_template(_asset_class(target_asset) if target_asset else "scene",template_override=body.template_override.strip())
        except ValueError as exc:
            raise HTTPException(422,str(exc)) from exc
    confirmed_asset_intents=[]
    asset_intent_version=body.asset_intent_version
    if target_asset_id:
        # Targeted Prompt drafts do not require a full intent confirmation,
        # but an existing intent snapshot still cannot be based on a stale
        # storyboard or asset manifest. This is a lineage guard only; it does
        # not alter the targeted Prompt compiler or its QA/status flow.
        ...
        if intent_state.get("sourceAssetManifestFingerprint") and intent_state.get("sourceAssetManifestFingerprint")!=current_manifest_fingerprint:
            raise HTTPException(409,{"message":"当前资产清单已经变化，请重新确认资产创作意图后再生成定向 Prompt。","manifest_stale":True})
        if intent_state.get("sourceStoryRevision") and intent_state.get("sourceStoryRevision")!=_asset_intent_story_revision(doc):
            raise HTTPException(409,{"message":"当前故事或分镜已经变化，请重新准备资产创作意图后再生成定向 Prompt。","story_stale":True})
        current_provenance=_asset_intent_provenance(doc)
        source_provenance=intent_state.get("sourceProvenance") if isinstance(intent_state.get("sourceProvenance"),dict) else None
        if source_provenance and (
            source_provenance.get("sourceFingerprint")!=current_provenance.get("sourceFingerprint")
            or source_provenance.get("sourceStoryboardVersionId")!=current_provenance.get("sourceStoryboardVersionId")
        ):
            raise HTTPException(409,{"message":"当前分镜版本已经变化，请重新准备或 rebase 资产创作意图后再生成定向 Prompt。","storyboard_stale":True})
    if not target_asset_id and asset_intent_version is not None:
        ...
        if source_provenance and (
            ...
            raise HTTPException(409,{"message":"当前分镜版本已经变化，请重新准备或 rebase 资产创作意图。","storyboard_stale":True})
        ...
        for asset in existing_assets:
            ...
            confirmed_asset_intents.append({
                ...
                "provenance":_asset_intent_provenance(doc,asset),
            })
    input_package={
        "project_id":project_id,
        "project_name":doc.get("name"),
        ...
        "handoff_provenance":_asset_intent_provenance(doc),
    ...
    # Every visual class, including fusion and shot outputs, uses the same
    # style-library compiler. The only class-specific additions are the
    # documented production invariants injected by that compiler.
    prompt_composition_mode="style_library"
    if operator_idea:
    ...
    if body.template_override:
        input_package["template_override"] = body.template_override.strip()
        input_package["workflow_constraint"] += f" 本次使用用户指定的 style library 模板 {body.template_override.strip()}；无效模板必须报错，不得创建自定义旧模板。"
    ...
    for card in prompt_output.get("assets",[]):
        ...
        try:
            prompt_cards.append(_canonical_prompt_card(
                {**card,"templateOverride":body.template_override} if body.template_override else card,
    ...
    if target_asset_id:
        for shot in shots:
            # A targeted Prompt draft is an enrichment pass.  Keep every
            # storyboard requirement, including one that the scoped regulator
            # response did not repeat, so a partial response cannot erase
            # accepted storyboard intent.
            shot["assetRequirements"]=_merge_story_asset_requirements(
                shot.get("assetRequirements"),
                req_map.get(str(shot.get("id")),[]),
                str(shot.get("id") or ""),
            )
    else:
        # The full Prompt pass may receive a regulator response that is
        # narrower than the accepted storyboard handoff.  Merge it instead of
        # replacing the storyboard-owned dependency list.
        for shot in shots:
            shot["assetRequirements"]=_merge_story_asset_requirements(
                shot.get("assetRequirements"),
                req_map.get(str(shot.get("id")),[]),
    ...
    approved_restore_keys=(
        ...
        "promptCompositionMode","promptCompilerVersion","promptTargetSkill","promptQaDecision",
        "promptStatus","mustPreserve","mustAvoid","generationChoiceStatus","generationStatus","externalGenerationRequired",
    )
    ...
    for card in prompt_output.get("assets",[]):
        ...
        if target_asset_id:
            ...
            enriched_cards.append({**card,"assetClass":cls,"targetSkill":target_skill,"promptVersion":asset.get("promptVersion"),"promptQaDecision":asset.get("promptQaDecision") or "Pending","generationChoiceStatus":"user-confirmation-required","generationStatus":asset.get("generationStatus") or "planned","externalGenerationRequired":cls in {"character","scene","prop","product","fusion","style","shot"},"uploadAllowed":cls in {"character","scene","prop","product","fusion","style","shot"},"imageQaRequired":cls in {"character","scene","prop","product","fusion","style","shot"},"assetAspectRatio":generation_profile["aspect_ratio"],"assetOutputFormat":"png","aspectRatio":generation_profile["aspect_ratio"],"outputFormat":"png","assetLayoutProfile":asset.get("assetLayoutProfile")})
            continue
        shots_for_card=[str(value) for value in card.get("relevantShots",[]) if value]
        canonical=_canonical_prompt_card({**card,"templateOverride":body.template_override} if body.template_override else card,asset,doc,shots_for_card,composition_mode=prompt_composition_mode)
        ...
        preserve=[str(value) for value in canonical.get("mustPreserve",[]) if value]
        avoid=[str(value) for value in canonical.get("mustAvoid",[]) if value]
        asset.update({"assetClass":cls,"assetRole":asset.get("assetRole") or card.get("role") or cls,"prompt":prompt,"promptVersion":prompt_version["id"],"promptPack":prompt_pack,"promptQuality":prompt_quality,"promptContractVersion":canonical["promptContractVersion"],"promptWorkflow":canonical["promptWorkflow"],"promptTargetSkill":target_skill,"promptQaDecision":"Pending","generationChoice":"user-confirmation-required","generationChoiceStatus":"user-confirmation-required","generationStatus":"planned","promptStatus":"prompt-draft","promptRelevantShots":shots_for_card,"mustPreserve":preserve,"mustAvoid":avoid,"externalGenerationRequired":cls in {"character","scene","prop","product","fusion","style","shot"}})
        if canonical.get("promptCompositionMode"):
            asset["promptCompositionMode"]=canonical["promptCompositionMode"]
        if canonical.get("promptCompilerVersion"):
            asset["promptCompilerVersion"]=canonical["promptCompilerVersion"]
        generation_profile=_apply_asset_generation_profile(asset,project_output_aspect_ratio)
        enriched_cards.append({**canonical,"assetClass":cls,"targetSkill":target_skill,"promptVersion":prompt_version["id"],"promptQaDecision":"Pending","generationChoiceStatus":"user-confirmation-required","generationStatus":"planned","externalGenerationRequired":cls in {"character","scene","prop","product","fusion","style","shot"},"uploadAllowed":cls in {"character","scene","prop","product","fusion","style","shot"},"imageQaRequired":cls in {"character","scene","prop","product","fusion","style","shot"},"assetAspectRatio":generation_profile["aspect_ratio"],"assetOutputFormat":"png","aspectRatio":generation_profile["aspect_ratio"],"outputFormat":"png","assetLayoutProfile":asset.get("assetLayoutProfile")})
    prompt_cards=[
        {
            **card,
    ...
    doc["assets"]=list(assets_by_id.values())
    missing_a=[str(asset["id"]) for asset in doc["assets"] if str(asset.get("grade","")) in {"A","A+"} and not _asset_readiness_with_prompt_contract(asset).get("ready")]
...
async def generate_fusion_prompt(project_id:str,body:FusionPromptRunCreate,request:Request):
    ...
    readiness={asset_id:_asset_readiness_with_prompt_contract(assets_by_id[asset_id]) for asset_id in source_ids}
    ...
    fusion_reference_roles=[
        {
            ...
            "scope":"该已确认基础资产的身份、结构、材质和比例",
            "purpose":f"锁定已确认的{assets_by_id[asset_id].get('name') or asset_id}视觉身份、结构、材质和比例",
        }
        for asset_id in source_ids
    ]
    raw_fusion_pack=result.get("promptPack") if isinstance(result.get("promptPack"),dict) else {}
    source_prompts=[str(assets_by_id[asset_id].get("prompt") or "").strip() for asset_id in source_ids]
    shot_action_parts=[
        str(shot.get("action") or "").strip(),
        str(shot.get("camera") or shot.get("framing") or shot.get("size") or "").strip(),
        str(shot.get("continuity") or "").strip(),
        str(shot.get("firstFrame") or "").strip(),
        str(shot.get("lastFrame") or "").strip(),
    ]
    raw_constraints=raw_fusion_pack.get("constraintsNegative")
    fusion_constraints=list(raw_constraints) if isinstance(raw_constraints,list) else []
    fusion_constraints.extend([
        "避免拼贴感和贴图感；保持同一透视、接触关系、投射阴影、环境光遮蔽和材质响应。",
        "明确角色、道具与场景的前中后景位置、相对尺度、遮挡关系和地面接触。",
    ])
    fusion_pack={
        "subjectTask":raw_fusion_pack.get("subjectTask") or shot.get("purpose") or plan.get("shot_intent") or "将已确认连接的角色、道具和场景组织为同一镜头中的统一画面",
        "compositionLayout":raw_fusion_pack.get("compositionLayout") or f"镜头{shot_id}中保持角色、道具和场景的前中后景关系、相对尺度、遮挡关系和地面接触清楚可见。",
        "visualStyleMaterials":raw_fusion_pack.get("visualStyleMaterials") or "；".join(value for value in source_prompts if value) or "统一已确认输入资产的视觉语言、材质响应、环境光和投射阴影。",
        "textLabels":raw_fusion_pack.get("textLabels") or "不添加可读文字、水印或标识。",
        "aspectRatioOutput":{"aspectRatio":normalize_aspect_ratio(doc.get("ratio") or "9:16","9:16"),"outputFormat":"png","source":"project_output"},
        "constraintsNegative":fusion_constraints,
        "referenceRoles":fusion_reference_roles,
        "actionContinuity":raw_fusion_pack.get("actionContinuity") or "；".join(value for value in shot_action_parts if value) or f"保持镜头{shot_id}的主体动作、摄影机关系和连续性。",
    }
    ...
    canonical=canonicalize_prompt_output(
        ...
        must_avoid=avoid,
        context={"shots":[shot],"references":fusion_reference_roles,"output_aspect_ratio":normalize_aspect_ratio(doc.get("ratio") or "9:16","9:16")},
    )
    fusion_issues=_local_visual_prompt_issues(
        fusion_asset,
        {"id":body.fusion_asset_id,"assetClass":"fusion","prompt":canonical["prompt"],"promptPack":canonical["promptPack"]},
        doc,
    )
    if fusion_issues:
        raise HTTPException(502,{"message":"融合 Prompt 未通过 style-library 规则校验。","issues":fusion_issues})
    ...
    fusion_asset.update({
        ...
        "prompt":prompt,"promptVersion":prompt_version["id"],"promptPack":result.get("promptPack") or {},"promptQuality":prompt_quality,"promptContractVersion":result.get("promptContractVersion") or PROMPT_CONTRACT_VERSION,"promptWorkflow":result.get("promptWorkflow") or PROMPT_WORKFLOW_ID,"promptTargetSkill":"video-fusion-production-director","promptRelevantShots":[shot_id],
        "promptQaDecision":"Pending","generationChoice":"user-confirmation-required","generationChoiceStatus":"user-confirmation-required","generationStatus":"planned","promptStatus":"prompt-draft","mustPreserve":[str(value) for value in result.get("mustPreserve",[]) if value],"mustAvoid":[str(value) for value in result.get("mustAvoid",[]) if value],"externalGenerationRequired":True,
    })
...
async def start_story_run(run_id:str,request:Request):
    ...
    try:
        ...
        def prepare_storyboard_result(raw_result:dict[str,Any],repair_attempts:int=0)->dict[str,Any]:
            prepared=raw_result if isinstance(raw_result,dict) else {}
            expected_source_version=str(input_package.get("source_script_version_id") or "")
            expected_source_hash=str(input_package.get("source_script_hash") or "")
            raw_source_version=str(raw_result.get("sourceScriptVersionId") or raw_result.get("source_script_version_id") or "") if isinstance(raw_result,dict) else ""
            raw_source_hash=str(raw_result.get("sourceScriptHash") or raw_result.get("source_script_hash") or "") if isinstance(raw_result,dict) else ""
            source_version_mismatch=bool(expected_source_version and raw_source_version and raw_source_version!=expected_source_version)
            source_hash_mismatch=bool(expected_source_hash and raw_source_hash and raw_source_hash!=expected_source_hash)
            ...
            if str(input_package.get("workflow_mode") or "") == "storyboard_from_source":
                ...
                source_mutation_detected=bool(str(raw_result.get("proposedScript") or "") and str(raw_result.get("proposedScript") or "") != str(input_package.get("current_script") or "")) if isinstance(raw_result,dict) else False
            ...
            prepared["sourceScriptPolicy"]="immutable" if str(input_package.get("workflow_mode") or "") == "storyboard_from_source" else "preserve_user_intent"
            source_script=str(input_package.get("current_script") or "")
            prepared["sourceScriptVersionId"]=expected_source_version or prepared.get("sourceScriptVersionId")
            prepared["sourceScriptHash"]=expected_source_hash or hashlib.sha256(source_script.encode("utf-8")).hexdigest()
            ...
            if source_mutation_detected:
                prepared["sourceScriptMutationDetected"]=True
            if source_version_mismatch:
                prepared["sourceScriptVersionMismatchDetected"]=True
            if source_hash_mismatch:
                prepared["sourceScriptHashMismatchDetected"]=True
            prepared["ruleProfile"]=input_package.get("rule_profile") or "legacy"
            prepared["ruleProfileVersion"]=input_package.get("rule_profile_version")
            prepared["ruleModules"]=input_package.get("rule_modules") or {}
            prepared["ruleSources"]=input_package.get("rule_sources") or []
            prepared.setdefault("assetHandoff",{})
            prepared["assetHandoff"]={
                **(prepared.get("assetHandoff") if isinstance(prepared.get("assetHandoff"),dict) else {}),
                "handoffVersion":"storyboard-handoff-v2",
                "sourceScriptVersionId":input_package.get("source_script_version_id"),
                "sourceStoryboardVersionId":None,
                "sourceStoryboardRunId":run_id,
                "sourceFingerprint":input_package.get("source_script_hash"),
                "sourceShotIds":[str(shot.get("id")) for shot in prepared.get("shots",[]) if isinstance(shot,dict) and shot.get("id")],
                "projectId":project_id,
                "targetGenerator":input_package.get("target_generator"),
                "returnExpected":"video-asset-regulator",
            }
            review=director_auto_review(prepared,input_package,repair_attempts)
            prepared["autoReview"]=review
            prepared["handoffCompatibility"]=review.get("handoffCompatibility") if isinstance(review.get("handoffCompatibility"),dict) else {}
            return prepared

        def validation_issues(prepared:dict[str,Any])->list[str]:
            combined=list(_validate_storyboard_output(prepared,input_package))
            review=prepared.get("autoReview") if isinstance(prepared.get("autoReview"),dict) else {}
            for item in review.get("issues") if isinstance(review.get("issues"),list) else []:
                if not isinstance(item,dict) or item.get("severity") != "blocking":
                    continue
                code=str(item.get("code") or "director_review_blocked")
            ...
            repair_input={
                **input_package,
                ...
                "contract_repair_context":{
                    ...
                    "autoReview":result.get("autoReview") if isinstance(result,dict) else {},
                    "shots":result.get("shots",[]) if isinstance(result,dict) else [],
                },
            }
            try:
                result=prepare_storyboard_result(await _run_storyboard_agent(request,project_id,repair_input),repair_attempts=1)
                issues=validation_issues(result)
        ...
        if issues:
            ...
            auto_review=result.get("autoReview") if isinstance(result.get("autoReview"),dict) else {}
            result["scriptAcceptanceAllowed"]=(
                ...
                and auto_review.get("scriptAcceptanceAllowed") is not False
            )
            result["blockingIssues"]=[{"code":issue.split(":",1)[0],"message":issue} for issue in issues]
        else:
            auto_review=result.get("autoReview") if isinstance(result.get("autoReview"),dict) else {}
            result["handoffStatus"]="needs_user_review" if auto_review.get("status")=="needs_user_review" else "ready"
            result["acceptanceAllowed"]=auto_review.get("acceptanceAllowed") is not False
            result["scriptAcceptanceAllowed"]=(str(input_package.get("workflow_mode") or "") != "storyboard_from_source" and auto_review.get("scriptAcceptanceAllowed") is not False)
        if repair_attempted:
...
async def accept_storyboard(run_id:str,body:StoryboardAcceptRequest,request:Request):
    ...
    proposed_shots=[_canonical_story_shot(item,doc.get("ratio") or "9:16") for item in output.get("shots",[]) if isinstance(item,dict)]
    output={**output,"shots":proposed_shots}
    spec=doc.setdefault("storySpec",{})
    spec["workflow_mode"]=workflow_mode
    if is_director_rule_profile(input_package.get("rule_profile")):
        spec["rule_profile"]=DIRECTOR_RULE_PROFILE
        spec["rule_profile_version"]=input_package.get("rule_profile_version") or director_rule_profile()["version"]
        spec["rule_modules"]=deepcopy(input_package.get("rule_modules") or {})
        spec["rule_sources"]=deepcopy(input_package.get("rule_sources") or director_rule_profile()["sources"])
    ...
    if body.scope in {"all","shots_only"} and proposed_shots:
        ...
        for s in proposed_shots:
            ...
            accepted_shot={**base,**{k:v for k,v in s.items() if k not in ("assetRequirements",)},**{"status":base.get("status") or "ready"}}
            accepted_shot["assetRequirements"]=_merge_story_asset_requirements(base.get("assetRequirements"),s.get("assetRequirements"),str(s.get("id") or ""))
            merged.append(accepted_shot)
        if body.shot_ids:
        ...
        storyboard_version_id=_story_version_id("storyboard",project_id,len(doc.get("storyboardVersions",[]))+1)
        provenance=_storyboard_handoff_provenance(doc,storyboard_version_id)
        output["assetHandoff"]={**(output.get("assetHandoff") if isinstance(output.get("assetHandoff"),dict) else {}),**provenance}
        output["handoffProvenance"]=provenance
        sbv={"id":storyboard_version_id,"parentId":row["source_script_version_id"],"scriptVersionId":doc["scriptVersions"][-1]["id"] if doc.get("scriptVersions") else None,"status":"active","shotIds":[s.get("id") for s in merged],"package":output,"provenance":provenance,"createdAt":utcnow(),"acceptedAt":utcnow()}
    ...
    try:
        reg_input={"project_id":project_id,"visualized_script":doc.get("script",""),"shots":doc.get("shots",[]),"asset_handoff":output.get("assetHandoff",{}),"handoff_provenance":output.get("handoffProvenance",{}),"existing_assets":doc.get("assets",[]),"stable_shot_ids":[s.get("id") for s in doc.get("shots",[])],"existing_qa_evidence":[]}
...
async def accept_regulator(run_id:str,request:Request):
    ...
    # The regulator remains the acceptance owner.  When it confirms a
    # storyboard handoff but omits an otherwise stable handoff asset from its
    # extraction list, materialize that declared asset instead of silently
    # losing the storyboard → intent → Prompt route.
    extraction=[dict(item) for item in output.get("assetExtraction",[]) if isinstance(item,dict)]
    extracted_by_id={str(item.get("id") or item.get("assetId") or item.get("asset_id") or "").strip() for item in extraction}
    for asset_id,item in handoff_by_id.items():
        if not asset_id or asset_id in extracted_by_id:
            continue
        extraction.append({
            "id":asset_id,
            "name":item.get("name") or item.get("assetName") or asset_id,
            "assetClass":item.get("assetClass") or "unknown",
            "priority":item.get("priority") or item.get("grade") or "B",
            "required":item.get("required",True),
            "role":item.get("productionRole") or item.get("role") or "",
            "source":"storyboard-handoff",
        })
        extracted_by_id.add(asset_id)
    output={**output,"assetExtraction":extraction}
    ...
    for shot in doc.get("shots",[]):
        ...
        merged_requirements=_merge_story_asset_requirements(shot.get("assetRequirements"),req_map.get(shot_id,[]),shot_id)
        shot["assetRequirements"]=merged_requirements
        final_req_map[shot_id]=merged_requirements
    ...
    now=utcnow()
    with database.connect() as connection:
        for shot_id,requirements in final_req_map.items():
            for requirement in requirements:
                asset_id=str(requirement.get("assetId") or "")
    ...
    for asset in doc.get("assets",[]):
        ...
        preview_seen.add(asset_id)
    handoff_receipt={"createdAssets":[str(item.get("id") or item.get("assetId") or item.get("asset_id")) for item in output.get("assetExtraction",[]) if isinstance(item,dict) and (item.get("id") or item.get("assetId") or item.get("asset_id")) and str(item.get("id") or item.get("assetId") or item.get("asset_id")) not in existing_ids],"updatedAssets":[str(item.get("id") or item.get("assetId") or item.get("asset_id")) for item in output.get("assetExtraction",[]) if isinstance(item,dict) and (item.get("id") or item.get("assetId") or item.get("asset_id")) and str(item.get("id") or item.get("assetId") or item.get("asset_id")) in existing_ids],"shotAssetEdges":sum(len(value) for value in final_req_map.values()),"referenceAssetEdges":reference_edge_count,"resolvedAssetIds":sorted(all_asset_ids & (set(reference_ids)|extracted_ids)),"unresolvedAssetIds":unresolved_refs,"awaitingReferenceAssets":awaiting_refs,"productionOrderPreview":production_preview,"fusionSlotStates":[{"shotId":shot.get("id"),"status":"awaiting_assets" if shot.get("assetRequirements") else "not_required"} for shot in doc.get("shots",[])],"provenance":storyboard_output.get("handoffProvenance") or _storyboard_handoff_provenance(doc),"status":"ready"}
    doc["assetHandoffReceipt"]=handoff_receipt
    doc["assetRegulator"]={"version":2,"status":"approved" if not missingA else "draft","auditedAt":utcnow(),"missingA":missingA,"dependencyVersion":"v02","handoffReceipt":handoff_receipt,"handoffProvenance":handoff_receipt["provenance"]}
    doc.setdefault("storyWorkflowRuns",[]).append({"runId":run_id,"acceptedAt":utcnow(),"step":"regulator"})
    _synchronise_fusion_slots(doc,create=bool(doc.get("assetPromptRuns")))
    next_revision=save_project_document(request,doc,rev)
    board=_sync_asset_board_after_document(database,project_id,doc,next_revision)
    _set_chain(database,run_id,active_step="succeeded",status="succeeded",regulator_output_json=database.encode(output))
    ...
    async def events():
        ...
        try:
            if profile["provider_type"]=="opencode":
                ...
                raise HTTPException(409,"当前 Agent 仅使用 OpenCode，请在设置中绑定 OpenCode。")
...
async def edit_image(body:ImageEdit,request:Request):
    del body,request
    raise HTTPException(410,"内部图片编辑已移除，请使用外部图像编辑后上传候选文件。")
...
def _asset_generation_profile(asset:dict[str,Any]|None,project_output_aspect_ratio:Any="9:16")->dict[str,Any]:
    asset=asset if isinstance(asset,dict) else {}
    return asset_generation_profile(_asset_class(asset),project_output_aspect_ratio)


def _asset_readiness_with_prompt_contract(asset:dict[str,Any]|None)->dict[str,Any]:
    """Apply the active style-library Prompt gate to visual readiness.
    ...
    if not isinstance(asset,dict):
        return report
    asset_class=canonical_asset_class(_asset_class(asset))
    if asset_class not in {"character","scene","prop","product","style","fusion","shot"}:
        return report
    prompt=str(asset.get("prompt") or "").strip()
    prompt_pack=asset.get("promptPack") or _asset_metadata(asset).get("prompt_pack") or {}
    if prompt and prompt_pack_has_generation_fields(asset_class,prompt_pack):
        return report
    missing=list(report.get("production_missing") or [])
    if "prompt" not in missing:
        missing.append("prompt")
    if "prompt_contract" not in missing:
        missing.append("prompt_contract")
    report["production_missing"]=missing
    report["production_ready"]=False
    if not prompt or report.get("next_action") in {"可入镜","完成 Prompt QA","补 Prompt"}:
        report["next_action"]="重新生成 style-library Prompt"
    return report
...
def _apply_asset_generation_profile(asset:dict[str,Any],project_output_aspect_ratio:Any="9:16")->dict[str,Any]:
    ...
    asset["assetOutputFormat"]="png"
    for retired_key in ("assetGenerationSize", "assetProviderAspectRatio", "providerAspectRatio", "provider_aspect_ratio"):
        asset.pop(retired_key, None)
    return profile
...
def _asset_intent_story_revision(doc:dict[str,Any])->str:
    ...
    """
    # Prompt preparation may create a derived fusion slot and the regulator
    # may enrich an existing requirement with reference-role metadata. Those
    # downstream projections are not storyboard edits, so keep only the
    # storyboard-owned requirement fields in this invalidation token.
    source_shots=[]
    for raw_shot in doc.get("shots") or []:
        if not isinstance(raw_shot,dict):
            continue
        shot=deepcopy(raw_shot)
        raw_requirements=shot.get("assetRequirements") or shot.get("asset_requirements") or []
        if isinstance(raw_requirements,list):
            stable_requirements=[]
            for requirement in raw_requirements:
                if not isinstance(requirement,dict):
                    continue
                asset_class=str(requirement.get("assetClass") or requirement.get("asset_class") or "").strip().lower()
                source_name=str(requirement.get("source") or "").strip().lower()
                if asset_class=="fusion" or source_name=="frameflow-fusion-slot":
                    continue
                stable_requirements.append({
                    key:deepcopy(requirement[key])
                    for key in ("shotId","shot_id","assetId","asset_id","assetClass","asset_class","role","priority","required","requiredReadiness")
                    if key in requirement
                })
            shot["assetRequirements"]=sorted(stable_requirements,key=lambda item:str(item.get("assetId") or item.get("asset_id") or ""))
            shot.pop("asset_requirements",None)
        source_shots.append(shot)
    source={
        "storySpec":doc.get("storySpec") or {},
        "script":str(doc.get("script") or ""),
        "scenes":doc.get("scenes") or [],
...
def _asset_intent_provenance(doc:dict[str,Any],asset:dict[str,Any]|None=None)->dict[str,Any]:
    """Lineage metadata only; it is not a visual Prompt input rule."""
    provenance=_storyboard_handoff_provenance(doc)
    if isinstance(asset,dict):
        provenance={**provenance,"sourceShotIds":_asset_intent_relevant_shots(doc,asset)}
    return provenance
...
def _asset_intent_manifest_fingerprint(doc:dict[str,Any])->str:
    ...
    for asset in doc.get("assets",[]):
        ...
        manifest.append({
            ...
            "assetClass":_asset_class(asset),
            # assetRole is often populated by the Prompt projection after the
            # handoff (for example, a missing role becomes the class label).
            # Use the source-side note/metadata role and do not let that
            # downstream default invalidate an otherwise unchanged intent.
            "assetRole":str(_asset_metadata(asset).get("asset_role") or _asset_metadata(asset).get("assetRole") or asset.get("note") or ""),
...
def _asset_intent_state(doc:dict[str,Any])->dict[str,Any]:
    ...
    if not isinstance(raw,dict):
        return {"assetIntentVersion":0,"sourceStoryRevision":None,"sourceAssetManifestFingerprint":None,"sourceProvenance":None,"entries":[],"updatedAt":None}
    ...
    return {
        ...
        "sourceProvenance":deepcopy(raw.get("sourceProvenance")) if isinstance(raw.get("sourceProvenance"),dict) else None,
        "entries":[item for item in entries if isinstance(item,dict) and item.get("assetId")],
        "updatedAt":raw.get("updatedAt"),
    }


def _asset_intent_entry_payload(asset:dict[str,Any],entry:dict[str,Any]|None,doc:dict[str,Any],source_provenance:dict[str,Any]|None=None)->dict[str,Any]:
    asset_id=str(asset.get("id") or "")
    asset_class=_asset_class(asset)
    ...
    asset_provenance=deepcopy(source_provenance) if isinstance(source_provenance,dict) else _asset_intent_provenance(doc,asset)
    asset_provenance["sourceShotIds"]=_asset_intent_relevant_shots(doc,asset)
    return {
        "assetId":asset_id,
        "assetClass":asset_class,
        "assetName":str(asset.get("name") or asset_id),
...
def _asset_intent_envelope(project_id:str,revision:int,doc:dict[str,Any])->dict[str,Any]:
    ...
    assets=[_asset_intent_entry_payload(asset,entries.get(str(asset.get("id"))),doc,state.get("sourceProvenance")) for asset in base_assets]
    ...
    source_provenance=state.get("sourceProvenance") if isinstance(state.get("sourceProvenance"),dict) else None
    provenance_stale=bool(source_provenance and (
        source_provenance.get("sourceFingerprint")!=current_provenance.get("sourceFingerprint")
        or source_provenance.get("sourceStoryboardVersionId")!=current_provenance.get("sourceStoryboardVersionId")
    ))
    stale=manifest_stale or story_stale or provenance_stale
    ...
    if provenance_stale:
        warnings.append("当前资产意图来自旧版分镜，需要重新准备或 rebase 后再生成 Prompt。")
    return {
        "project_id":project_id,
        "revision":revision,
        ...
        "sourceProvenance":source_provenance,
        "currentProvenance":current_provenance,
        "manifestStale":stale,
        "assets":assets,
...
def _generation_reference_snapshot(database:Database,project_id:str,logical_asset_id:str)->list[dict[str,Any]]:
    """Return the ordered reference-role snapshot used by asset QA.

    This is reference authority data, not image-provider execution state.  It
    remains available for audit and continuity checks after internal image
...
def _fusion_connection_reference_roles(doc:dict[str,Any],asset:dict[str,Any])->list[dict[str,Any]]:
    """Build registration reference roles from confirmed fusion inputs."""
    ...
    if not source_ids or sorted(source_ids)!=sorted(run_source_ids):
        return []
    assets={str(item.get("id")):item for item in doc.get("assets",[]) if isinstance(item,dict) and item.get("id")}
    prompt_pack=asset.get("promptPack") if isinstance(asset.get("promptPack"),dict) else {}
    raw_roles=prompt_pack.get("referenceRoles") or prompt_pack.get("reference_roles") or []
    ...
    for source_id in source_ids:
        ...
        normalized.append({
            ...
            "notes":str(raw.get("scope") or raw.get("purpose") or "已确认融合输入资产角色"),
        })
    return normalized
...
def _fusion_gate(doc:dict[str,Any],asset:dict[str,Any],database:Database,project_id:str,references:list[dict[str,Any]]|None=None)->dict[str,Any]:
    """Check that confirmed fusion inputs and their reference roles are ready.

    This is a production/readiness gate, not a Prompt compiler.  It only
    consumes explicit fusion source metadata and canonical reference-role
    records; Prompt Pack fields remain limited to the new style-library
    semantic contract.
    """
    metadata=_asset_metadata(asset)
...
def _asset_prerequisite_dependencies(
    ...
    *,
    ...
    """Collect prerequisites from explicit workflow data only.

    Visual Prompt Packs are not a dependency database. This keeps historical
    Prompt fields from re-entering the active production gate.
    """
    asset_id=str(asset.get("id") or "")
    known_ids=known_asset_ids if known_asset_ids is not None else {
        str(item.get("id")) for item in doc.get("assets",[]) if isinstance(item,dict) and item.get("id")
    }
    ...
    metadata=_asset_metadata(asset)
    explicit_ids=(
        asset.get("prerequisiteAssetIds")
        or asset.get("prerequisite_asset_ids")
        or metadata.get("prerequisite_asset_ids")
        or metadata.get("prerequisiteAssetIds")
        or []
    )
    for dependency_id in explicit_ids if isinstance(explicit_ids,list) else []:
        add(dependency_id,"资产元数据声明的前置资产","asset_metadata")
...
def _library_payload(database:Database,project_id:str,doc:dict[str,Any])->dict[str,Any]:
    ...
    for asset in doc.get("assets",[]):
        ...
        readiness=_asset_readiness_with_prompt_contract(readiness_asset)
        prompt_value=str(asset.get("prompt") or "").strip()
        had_prompt=bool(prompt_value)
        prompt_pack=asset.get("promptPack") or _asset_metadata(asset).get("prompt_pack") or {}
        visual_asset=asset_class in ASSET_BOARD_VISUAL_CLASSES
        # A historical Prompt without a complete new semantic pack is not a
        # usable style-library Prompt.  Keep the stored text for audit, but do
        # not compile or expose it as a new Prompt/fallback in the active
        # library projection.
        legacy_prompt_needs_regeneration=bool(visual_asset and had_prompt and not prompt_pack_has_generation_fields(asset_class,prompt_pack))
        if legacy_prompt_needs_regeneration:
            readiness_asset["prompt"]=""
            readiness=_asset_readiness_with_prompt_contract(readiness_asset)
        ...
        canonical_prompt=canonicalize_prompt_output(
            asset_class,
            prompt_pack,
            "" if legacy_prompt_needs_regeneration else prompt_value,
        ...
        if legacy_prompt_needs_regeneration:
            canonical_prompt["needsRegeneration"]=True
            canonical_prompt["promptPack"]={}
            canonical_prompt["promptQuality"]=assess_prompt_pack(asset_class,canonical_prompt["promptPack"],"",needs_regeneration=True)
        prompt_value=canonical_prompt["prompt"] if had_prompt and not legacy_prompt_needs_regeneration else ""
        prompt_quality=canonical_prompt["promptQuality"] if had_prompt else asset.get("promptQuality")
        item={**asset,"assetClass":asset_class,"assetAspectRatio":generation_profile["aspect_ratio"],"assetAspectRatioSource":generation_profile["source"],"assetLayoutProfile":asset.get("assetLayoutProfile") or asset_layout_profile(asset_class),"assetOutputFormat":"png","aspectRatio":generation_profile["aspect_ratio"],"outputFormat":"png","externalGenerationRequired":asset_class in ASSET_BOARD_VISUAL_CLASSES,"uploadAllowed":asset_class in ASSET_BOARD_VISUAL_CLASSES,"imageQaRequired":asset_class in ASSET_BOARD_VISUAL_CLASSES,"promptConsistencyReview":asset.get("promptConsistencyReview"),"needsRegeneration":bool(canonical_prompt.get("needsRegeneration") or legacy_prompt_needs_regeneration),"prompt":prompt_value,"copyablePrompt":prompt_value,"promptPack":canonical_prompt["promptPack"],"promptQuality":prompt_quality,"promptContractVersion":canonical_prompt["promptContractVersion"],"promptWorkflow":canonical_prompt["promptWorkflow"],"assetMetadata":_asset_metadata(asset),"readiness":readiness,"registered_ready":readiness.get("registered_ready",False),"production_ready":readiness.get("production_ready",False),"next_action":readiness.get("next_action"),"artifacts":linked_artifacts,"artifact_count":len(linked_artifacts),"active_artifact_count":len(live_linked_artifacts),"archived_artifact_count":len(linked_artifacts)-len(live_linked_artifacts),"versions":versions_by_logical.get(logical_id,[]),"promptVersions":prompts_by_logical.get(logical_id,[]),"references":references,"dependencies":dependencies,"comparisons":comparisons}
        for retired_key in ("promptFieldOrder","prompt_field_order","imageGenerationEligible","image_generation_eligible","assetGenerationSize","asset_generation_size","assetProviderAspectRatio","asset_provider_aspect_ratio","imageExecutionPrompt","image_execution_prompt"):
...
ASSET_BOARD_VISUAL_CLASSES={"character","scene","prop","product","style","fusion","shot"}
...
def _asset_board_from_document(database:Database,project_id:str,doc:dict[str,Any],project_revision:int,previous:dict[str,Any]|None=None,artifact_rows:list[Any]|None=None)->dict[str,Any]:
    ...
    for asset in visual_assets:
        ...
        readiness=_asset_readiness_with_prompt_contract(asset); label=str(asset.get("name") or asset_id)
        config={"asset_class":class_name,"asset_aspect_ratio":generation_profile["aspect_ratio"],"asset_aspect_ratio_source":generation_profile["source"],"asset_output_format":"png","grade":asset.get("grade") or readiness.get("grade") or "B","readiness_status":readiness.get("status"),"required":bool(readiness.get("required")),"group_id":f"group:{group_key}","archived":False}
        if isinstance(old.get("config"),dict): config={**config,**old["config"]}
        for retired_key in ("asset_generation_size","asset_provider_aspect_ratio","provider_aspect_ratio","image_generation_eligible"):
            config.pop(retired_key,None)
        config.pop("prompt_field_order",None)
        config["asset_output_format"]="png"
        ...
        if prompt or production_draft_active or (class_name=="fusion" and _is_fusion_slot(asset)):
            ...
            handoff_config={"asset_id":asset_id,"source_type":"external-upload","group_id":f"group:{group_key}","archived":False,"artifact_id":preview_id or None,"artifact_url":preview_url or None,"artifact_status":str(preview["status"]) if preview else None,"artifact_qa_decision":str(preview["qa_decision"]) if preview else None,"artifact_source_type":str(preview["source_type"] or "upload") if preview else None,"artifact_active":bool(preview and str(preview["id"]) in active_version_by_artifact),"artifact_version_id":active_version_by_artifact.get(str(preview["id"])) if preview else None}
            ...
            handoff_config.update({"prompt_card":True,"production_draft":production_draft_active,"asset_class":class_name,"asset_aspect_ratio":generation_profile["aspect_ratio"],"asset_aspect_ratio_source":generation_profile["source"],"asset_output_format":"png","external_generation_required":class_name in ASSET_BOARD_VISUAL_CLASSES,"upload_allowed":class_name in ASSET_BOARD_VISUAL_CLASSES,"image_qa_required":class_name in ASSET_BOARD_VISUAL_CLASSES,"prompt":prompt,"copyable_prompt":prompt,"prompt_pack":canonical_prompt["promptPack"],"prompt_quality":canonical_prompt["promptQuality"] if raw_prompt else asset.get("promptQuality") or {},"prompt_contract_version":canonical_prompt["promptContractVersion"],"prompt_workflow":canonical_prompt["promptWorkflow"],"prompt_version":asset.get("promptVersion") or metadata.get("prompt_version"),"prompt_qa_decision":asset.get("promptQaDecision") or "Pending","generation_choice":asset.get("generationChoice") or "user-confirmation-required","generation_status":asset.get("generationStatus") or "planned","target_skill":asset.get("promptTargetSkill") or asset.get("skill") or _asset_prompt_skill(class_name),"relevant_shots":asset.get("promptRelevantShots") or [],"must_preserve":asset.get("mustPreserve") or metadata.get("must_preserve") or [],"must_avoid":asset.get("mustAvoid") or metadata.get("must_avoid") or [],"fusion_prompt_source":asset.get("fusionPromptSource"),"fusion_prompt_state":asset.get("fusionPromptState"),"fusion_prompt_stale":bool(asset.get("fusionPromptStale")),"fusion_prompt_stale_reason":asset.get("fusionPromptStaleReason"),"fusion_plan":asset.get("fusionPlan") or {}})
            for retired_key in ("prompt_field_order","image_generation_eligible","asset_generation_size","asset_provider_aspect_ratio","image_execution_prompt"):
                handoff_config.pop(retired_key,None)
            if fusion_gate is not None:
...
async def create_prompt_version(project_id:str,logical_asset_id:str,body:PromptCreateRequest,request:Request):
    ...
    requested_mode="style_library"
    canonical=_canonical_prompt_for_asset(doc,asset,body.prompt,composition_mode=requested_mode,preserve_clean_prompt=True)
    prompt=str(canonical["prompt"] or "").strip()
    if cls in {"character","scene","prop","product","style","fusion","shot"} and (canonical.get("needsRegeneration") or not prompt):
        raise HTTPException(422,"旧版或不完整的 Prompt 不能直接创建新版记录，请重新生成 style-library Prompt。")
    ...
    asset.update({"prompt":prompt,"promptVersion":pv["id"],"promptQaDecision":"Pending","generationChoice":"user-confirmation-required","promptPack":canonical["promptPack"],"promptQuality":canonical["promptQuality"],"promptContractVersion":canonical["promptContractVersion"],"promptWorkflow":canonical["promptWorkflow"],"mustPreserve":canonical.get("mustPreserve") or asset.get("mustPreserve") or [],"mustAvoid":canonical.get("mustAvoid") or asset.get("mustAvoid") or []})
    if canonical.get("promptCompositionMode"):
        asset["promptCompositionMode"] = canonical["promptCompositionMode"]
    if canonical.get("promptCompilerVersion"):
        asset["promptCompilerVersion"] = canonical["promptCompilerVersion"]
...
async def update_asset_metadata(project_id:str,logical_asset_id:str,body:AssetMetadataUpdate,request:Request):
    ...
    if prompt_fields_changed:
        ...
        submitted_prompt_pack=body.prompt_pack if body.prompt_pack is not None else asset.get("promptPack") or metadata.get("prompt_pack") or {}
        canonical_asset_class_name=canonical_asset_class(asset_class)
        submitted_prompt_is_new_visual_pack=canonical_asset_class_name in {"character","scene","prop","product","style","fusion","shot"} and prompt_pack_has_generation_fields(canonical_asset_class_name,submitted_prompt_pack)
        try:
            canonical_prompt=_canonical_prompt_for_asset(
                doc,
                asset,
                body.prompt if body.prompt is not None else str(asset.get("prompt") or ""),
                prompt_pack=submitted_prompt_pack,
                ...
                # Every saved visual edit passes through the local compiler.
                # A complete new Pack may preserve a clean full-text edit; a
                # short or incomplete textarea value is rebuilt from the Pack
                # so ratio/layout/output invariants remain present. An old
                # Pack can never become a fallback.
                strict_clean=False,
                preserve_clean_prompt=submitted_prompt_is_new_visual_pack and prompt_changed,
            )
        except ValueError as exc:
            raise HTTPException(422,{"message":"Prompt 重写未形成完整的新稿，未保存当前修改。","issues":[str(exc)]}) from exc
        if asset_class in {"character","scene","prop","product","style","fusion","shot"} and (canonical_prompt.get("needsRegeneration") or not str(canonical_prompt.get("prompt") or "").strip()):
            raise HTTPException(422,"旧版或不完整的 Prompt 不能直接保存，请重新生成 style-library Prompt。")
    latest_prompt=None
    if prompt_changed:
    ...
    with database.connect() as mutation_connection:
        if prompt_changed:
            ...
            asset["mustPreserve"]=canonical_prompt.get("mustPreserve") or asset.get("mustPreserve") or []
            asset["mustAvoid"]=canonical_prompt.get("mustAvoid") or asset.get("mustAvoid") or []
...
async def manual_production_approval(project_id:str,logical_asset_id:str,body:AssetManualProductionApproval,request:Request):
    ...
    base_readiness=_asset_readiness_with_prompt_contract(base_asset)
    with database.connect() as connection:
        active_version=connection.execute(
